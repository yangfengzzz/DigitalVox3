...
//!
template <typename T> struct BoundingBoxRayIntersection {
...
public:
  ...
  static_assert(std::is_floating_point<T>::value, "BoundingBox only can be instantiated with floating point types");
  ...
  template <typename U = T> std::enable_if_t<(N > 1), U> height() const;

  //! Returns depth of the box.
  template <typename U = T> std::enable_if_t<(N > 2), U> depth() const;
  ...
  template <typename U> BoundingBox<U, N> castTo() const;
};
