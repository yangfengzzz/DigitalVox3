...
//! \tparam N - Dimension.
//!
template <typename T, size_t N> class NearestArraySampler final {
public:
  static_assert(N > 0, "Dimension should be greater than 0");

  using ScalarType = typename GetScalarType<T>::value;

  static_assert(std::is_floating_point<ScalarType>::value, "NearestArraySampler only can be instantiated with floating "
                                                           "point types");
  ...
  //!
  explicit NearestArraySampler(const ArrayView<const T, N> &view, const VectorType &gridSpacing,
...
template <typename T> using NearestArraySampler1 = NearestArraySampler<T, 1>;

template <typename T> using NearestArraySampler2 = NearestArraySampler<T, 2>;

template <typename T> using NearestArraySampler3 = NearestArraySampler<T, 3>;
...
//! \tparam N - Dimension.
//!
template <typename T, size_t N> class LinearArraySampler final {
public:
  static_assert(N > 0, "N should be greater than 0");

  using ScalarType = typename GetScalarType<T>::value;

  static_assert(std::is_floating_point<ScalarType>::value, "LinearArraySampler only can be instantiated with floating "
                                                           "point types");
  ...
  //! \param[in]  gridOrigin  The grid origin.
  //!
  explicit LinearArraySampler(const ArrayView<const T, N> &view, const VectorType &gridSpacing,
                              const VectorType &gridOrigin);
  ...
  void getCoordinatesAndWeights(const VectorType &pt, std::array<CoordIndexType, kFlatKernelSize> &indices,
  ...
  //! given point.
  void getCoordinatesAndGradientWeights(const VectorType &pt, std::array<CoordIndexType, kFlatKernelSize> &indices,
...
template <typename T> using LinearArraySampler1 = LinearArraySampler<T, 1>;

template <typename T> using LinearArraySampler2 = LinearArraySampler<T, 2>;

template <typename T> using LinearArraySampler3 = LinearArraySampler<T, 3>;
...
//! \tparam N - Dimension.
//!
template <typename T, size_t N, typename CubicInterpolationOp> class CubicArraySampler final {
public:
  static_assert(N > 0, "N should be greater than 0");

  using ScalarType = typename GetScalarType<T>::value;

  static_assert(std::is_floating_point<ScalarType>::value, "CubicArraySampler only can be instantiated with floating "
                                                           "point types");
  ...
  //! \param[in]  gridOrigin  The grid origin.
  //!
  explicit CubicArraySampler(const ArrayView<const T, N> &view, const VectorType &gridSpacing,
                             const VectorType &gridOrigin);
...
template <typename T> struct CatmullRom {
  using ScalarType = typename GetScalarType<T>::value;
...
template <typename T> struct MonotonicCatmullRom {
  using ScalarType = typename GetScalarType<T>::value;
...
template <typename T> using CatmullRomArraySampler1 = CubicArraySampler<T, 1, CatmullRom<T>>;

template <typename T> using CatmullRomArraySampler2 = CubicArraySampler<T, 2, CatmullRom<T>>;

template <typename T> using CatmullRomArraySampler3 = CubicArraySampler<T, 3, CatmullRom<T>>;

template <typename T> using MonotonicCatmullRomArraySampler1 = CubicArraySampler<T, 1, MonotonicCatmullRom<T>>;

template <typename T> using MonotonicCatmullRomArraySampler2 = CubicArraySampler<T, 2, MonotonicCatmullRom<T>>;
