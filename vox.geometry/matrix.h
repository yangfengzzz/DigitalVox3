...
class Matrix final : public MatrixExpression<T, Rows, Cols, Matrix<T, Rows, Cols>>,
                     public MatrixDenseBase<T, Rows, Cols, Matrix<T, Rows, Cols>> {
public:
  static_assert(isMatrixSizeStatic<Rows, Cols>(), "This class should be a static-sized matrix.");
  ...
  constexpr Matrix(const_reference first, Args... rest) : _elements{{first, static_cast<value_type>(rest)...}} {}

  template <size_t R, size_t C, typename E> Matrix(const MatrixExpression<T, R, C, E> &expression);
...
public:
  ...
  constexpr explicit Matrix(const T &x_) : x(x_) {}
...
public:
  ...
  constexpr Matrix(const Matrix<T, 2, 1> &xy_, const T &z_) : x(xy_.x), y(xy_.y), z(z_) {}

  constexpr Matrix(const T &x_, const T &y_, const T &z_) : x(x_), y(y_), z(z_) {}
...
public:
  ...
  constexpr Matrix(const T &x_, const T &y_, const T &z_, const T &w_) : x(x_), y(y_), z(z_), w(w_) {}

  constexpr Matrix(const Matrix &other) : x(other.x), y(other.y), z(other.z), w(other.w) {}
...
class Matrix<T, kMatrixSizeDynamic, kMatrixSizeDynamic> final
    : public MatrixExpression<T, kMatrixSizeDynamic, kMatrixSizeDynamic,
      ...
      public MatrixDenseBase<T, kMatrixSizeDynamic, kMatrixSizeDynamic,
                             Matrix<T, kMatrixSizeDynamic, kMatrixSizeDynamic>> {
public:
  using value_type = T;
  using reference = T &;
  ...
  using iterator = pointer;
  using const_iterator = const_pointer;
  using MatrixDenseBase<T, kMatrixSizeDynamic, kMatrixSizeDynamic,
...
class Matrix<T, kMatrixSizeDynamic, 1> final
    : public MatrixExpression<T, kMatrixSizeDynamic, 1, Matrix<T, kMatrixSizeDynamic, 1>>,
      public MatrixDenseBase<T, kMatrixSizeDynamic, 1, Matrix<T, kMatrixSizeDynamic, 1>> {
public:
  using value_type = T;
  using reference = T &;
  using const_reference = const T &;
  ...
  using iterator = pointer;
  using const_iterator = const_pointer;
  using MatrixDenseBase<T, kMatrixSizeDynamic, 1, Matrix<T, kMatrixSizeDynamic, 1>>::copyFrom;
...
template <typename T, size_t Rows> using Vector = Matrix<T, Rows, 1>;
...
template <typename T> using MatrixMxN = Matrix<T, kMatrixSizeDynamic, kMatrixSizeDynamic>;
...
template <typename T> using VectorN = Matrix<T, kMatrixSizeDynamic, 1>;
...
template <typename T, size_t Rows, size_t Cols> struct GetScalarType<Matrix<T, Rows, Cols>> { using value = T; };
