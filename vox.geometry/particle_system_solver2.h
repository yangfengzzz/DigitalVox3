...
//!
template <typename DerivedBuilder> class ParticleSystemSolverBuilderBase2 {
public:
...
template <typename T> T &ParticleSystemSolverBuilderBase2<T>::withRadius(double radius) {
  _radius = radius;
  return static_cast<T &>(*this);
}

template <typename T> T &ParticleSystemSolverBuilderBase2<T>::withMass(double mass) {
  _mass = mass;
  return static_cast<T &>(*this);
}
