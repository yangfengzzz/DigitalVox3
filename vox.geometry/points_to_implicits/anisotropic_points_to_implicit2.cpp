...
#include "../levelset_solvers/fmm_level_set_solver2.h"
#include "../point_searchers/point_kdtree_searcher.h"
...
AnisotropicPointsToImplicit2::AnisotropicPointsToImplicit2(double kernelRadius, double cutOffDensity,
                                                           double positionSmoothingFactor, size_t minNumNeighbors,
                                                           bool isOutputSdf)
    : _kernelRadius(kernelRadius), _cutOffDensity(cutOffDensity), _positionSmoothingFactor(positionSmoothingFactor),
      _minNumNeighbors(minNumNeighbors), _isOutputSdf(isOutputSdf) {}

void AnisotropicPointsToImplicit2::convert(const ConstArrayView1<Vector2D> &points, ScalarGrid2 *output) const {
  if (output == nullptr) {
  ...
  temp->fill([&](const Vector2D &x) {
    double sum = 0.0;
    meanNeighborSearcher2.forEachNearbyPoint(x, r, [&](size_t i, const Vector2D &neighborPosition) {
  ...
  if (_isOutputSdf) {
    ...
    JET_INFO << "Completed Reinitialization.";
  } else {
    temp->swap(output);
  }
