...
void SphericalPointsToImplicit2::convert(const ConstArrayView1<Vector2D> &points, ScalarGrid2 *output) const {
  if (output == nullptr) {
  ...
  temp->fill([&](const Vector2D &x) {
    ...
    neighborSearcher->forEachNearbyPoint(
        x, 2.0 * _radius, [&](size_t, const Vector2D &xj) { minDist = std::min(minDist, (x - xj).length()); });
