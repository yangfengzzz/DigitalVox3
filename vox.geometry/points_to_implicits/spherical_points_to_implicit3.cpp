...
void SphericalPointsToImplicit3::convert(const ConstArrayView1<Vector3D> &points, ScalarGrid3 *output) const {
  if (output == nullptr) {
  ...
  temp->fill([&](const Vector3D &x) {
    ...
    neighborSearcher->forEachNearbyPoint(
        x, 2.0 * _radius, [&](size_t, const Vector3D &xj) { minDist = std::min(minDist, (x - xj).length()); });
