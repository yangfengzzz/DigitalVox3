...
VolumeParticleEmitter2::VolumeParticleEmitter2(ImplicitSurface2Ptr implicitSurface, const BoundingBox2D &maxRegion,
...
void VolumeParticleEmitter2::emit(const ParticleSystemData2Ptr &particles, Array1<Vector2D> *newPositions,
                                  Array1<Vector2D> *newVelocities) {
  if (!_implicitSurface) {
    return;
  }
  ...
  if (_allowOverlapping || _isOneShot) {
    ...
    PointHashGridSearcher2 neighborSearcher(Vector2UZ(kDefaultHashGridResolution, kDefaultHashGridResolution),
                                            2.0 * _spacing);
