...
PointParticleEmitter2::PointParticleEmitter2(const Vector2D &origin, const Vector2D &direction, double speed,
    ...
    : _rng(seed), _maxNumberOfNewParticlesPerSecond(maxNumOfNewParticlesPerSec),
      _maxNumberOfParticles(maxNumOfParticles), _origin(origin), _direction(direction), _speed(speed),
...
void PointParticleEmitter2::onUpdate(double currentTimeInSeconds, double timeIntervalInSeconds) {
  ...
  auto newMaxTotalNumberOfEmittedParticles = static_cast<size_t>(std::ceil(
      (elapsedTimeInSeconds + timeIntervalInSeconds) * static_cast<double>(_maxNumberOfNewParticlesPerSecond)));
...
void PointParticleEmitter2::emit(Array1<Vector2D> *newPositions, Array1<Vector2D> *newVelocities,
...
PointParticleEmitter2Ptr PointParticleEmitter2::Builder::makeShared() const {
  return std::shared_ptr<PointParticleEmitter2>(
      new PointParticleEmitter2(_origin, _direction, _speed, _spreadAngleInDegrees, _maxNumberOfNewParticlesPerSecond,
                                _maxNumberOfParticles, _seed),
