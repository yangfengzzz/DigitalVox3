...
template <typename T> inline T smearedHeavisideSdf(T phi) {
  if (phi > 1.5) {
    ...
    if (phi < -1.5) {
      ...
      return half<T>() + phi * oneThird<T>() + half<T>() * invPi<T>() * std::sin(pi<T>() * phi / oneAndHalf<T>());
    }
  }
}
...
template <typename T> inline T smearedDeltaSdf(T phi) {
  if (std::fabs(phi) > 1.5) {
    ...
    return oneThird<T>() + oneThird<T>() * std::cos(pi<T>() * phi / oneAndHalf<T>());
  }
}
...
template <typename T> T fractionInside(T phiBottomLeft, T phiBottomRight, T phiTopLeft, T phiTopRight) {
  int inside_count =
      (phiBottomLeft < 0 ? 1 : 0) + (phiTopLeft < 0 ? 1 : 0) + (phiBottomRight < 0 ? 1 : 0) + (phiTopRight < 0 ? 1 : 0);
