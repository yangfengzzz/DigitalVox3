...
template <typename T, size_t N> void ListQueryEngine<T, N>::add(const ConstArrayView1<T> &items) {
  _items.append(items);
}

template <typename T, size_t N>
bool ListQueryEngine<T, N>::intersects(const BoundingBox<double, N> &box,
...
template <typename T, size_t N>
bool ListQueryEngine<T, N>::intersects(const Ray<double, N> &ray, const RayIntersectionTestFunc<T, N> &testFunc) const {
  for (const auto &item : _items) {
...
template <typename T, size_t N>
void ListQueryEngine<T, N>::forEachIntersectingItem(const BoundingBox<double, N> &box,
...
template <typename T, size_t N>
void ListQueryEngine<T, N>::forEachIntersectingItem(const Ray<double, N> &ray,
...
template <typename T, size_t N>
ClosestIntersectionQueryResult<T, N>
ListQueryEngine<T, N>::closestIntersection(const Ray<double, N> &ray,
...
template <typename T, size_t N>
NearestNeighborQueryResult<T, N>
ListQueryEngine<T, N>::nearest(const Vector<double, N> &pt,
