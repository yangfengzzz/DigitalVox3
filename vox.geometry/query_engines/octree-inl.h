...
template <typename T> typename Octree<T>::Iterator Octree<T>::begin() { return _items.begin(); }

template <typename T> typename Octree<T>::Iterator Octree<T>::end() { return _items.end(); }

template <typename T> typename Octree<T>::ConstIterator Octree<T>::begin() const { return _items.begin(); }

template <typename T> typename Octree<T>::ConstIterator Octree<T>::end() const { return _items.end(); }

template <typename T> size_t Octree<T>::numberOfItems() const { return _items.size(); }

template <typename T> const T &Octree<T>::item(size_t i) const { return _items[i]; }

template <typename T> size_t Octree<T>::numberOfNodes() const { return _nodes.size(); }
...
template <typename T> const BoundingBox3D &Octree<T>::boundingBox() const { return _bbox; }

template <typename T> size_t Octree<T>::maxDepth() const { return _maxDepth; }
...
Octree<T>::closestIntersection(const Ray3D &ray, const GetRayIntersectionFunc3<T> &testFunc, size_t nodeIdx,
                               const BoundingBox3D &bound, ClosestIntersectionQueryResult3<T> best) const {
  if (!bound.intersects(ray)) {
    return best;
  }
  ...
  if (node.firstChild != kMaxSize) {
    for (int i = 0; i < 8; ++i) {
      best = closestIntersection(ray, testFunc, node.firstChild + i, BoundingBox3D(bound.corner(i), bound.midPoint()),
                                 best);
    }
  }
