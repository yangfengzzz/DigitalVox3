...
template <typename T, size_t N> void Bvh<T, N>::Node::initLeaf(size_t it, const BoundingBox<double, N> &b) {
...
void Bvh<T, N>::Node::initInternal(uint8_t axis, size_t c, const BoundingBox<double, N> &b) {
  flags = axis;
  child = c;
  bound = b;
}

template <typename T, size_t N> bool Bvh<T, N>::Node::isLeaf() const { return flags == static_cast<char>(N); }

//

template <typename T, size_t N> Bvh<T, N>::Bvh() {}

template <typename T, size_t N>
void Bvh<T, N>::build(const ConstArrayView1<T> &items, const ConstArrayView1<BoundingBox<double, N>> &itemsBounds) {
  _items = items;
...
template <typename T, size_t N> void Bvh<T, N>::clear() {
  _bound = BoundingBox<double, N>{};
  _items.clear();
  _itemBounds.clear();
  _nodes.clear();
...
template <typename T, size_t N>
inline NearestNeighborQueryResult<T, N>
Bvh<T, N>::nearest(const Vector<double, N> &pt, const NearestNeighborDistanceFunc<T, N> &distanceFunc) const {
...
template <typename T, size_t N>
inline bool Bvh<T, N>::intersects(const BoundingBox<double, N> &box,
...
template <typename T, size_t N>
inline bool Bvh<T, N>::intersects(const Ray<double, N> &ray, const RayIntersectionTestFunc<T, N> &testFunc) const {
...
template <typename T, size_t N>
inline void Bvh<T, N>::forEachIntersectingItem(const BoundingBox<double, N> &box,
...
template <typename T, size_t N>
inline void Bvh<T, N>::forEachIntersectingItem(const Ray<double, N> &ray, const RayIntersectionTestFunc<T, N> &testFunc,
                                               const IntersectionVisitorFunc<T> &visitorFunc) const {
  if (!_bound.intersects(ray)) {
    return;
  }
...
template <typename T, size_t N>
inline ClosestIntersectionQueryResult<T, N>
Bvh<T, N>::closestIntersection(const Ray<double, N> &ray, const GetRayIntersectionFunc<T, N> &testFunc) const {
...
template <typename T, size_t N> const BoundingBox<double, N> &Bvh<T, N>::boundingBox() const { return _bound; }

template <typename T, size_t N> typename Bvh<T, N>::iterator Bvh<T, N>::begin() { return _items.begin(); }

template <typename T, size_t N> typename Bvh<T, N>::iterator Bvh<T, N>::end() { return _items.end(); }

template <typename T, size_t N> typename Bvh<T, N>::const_iterator Bvh<T, N>::begin() const { return _items.begin(); }

template <typename T, size_t N> typename Bvh<T, N>::const_iterator Bvh<T, N>::end() const { return _items.end(); }

template <typename T, size_t N> size_t Bvh<T, N>::numberOfItems() const { return _items.length(); }

template <typename T, size_t N> const T &Bvh<T, N>::item(size_t i) const { return _items[i]; }

template <typename T, size_t N> size_t Bvh<T, N>::numberOfNodes() const { return _nodes.length(); }

template <typename T, size_t N> std::pair<size_t, size_t> Bvh<T, N>::children(size_t i) const {
  if (isLeaf(i)) {
...
template <typename T, size_t N> bool Bvh<T, N>::isLeaf(size_t i) const { return _nodes[i].isLeaf(); }

template <typename T, size_t N> const BoundingBox<double, N> &Bvh<T, N>::nodeBound(size_t i) const {
  return _nodes[i].bound;
}

template <typename T, size_t N> typename Bvh<T, N>::iterator Bvh<T, N>::itemOfNode(size_t i) {
  if (isLeaf(i)) {
...
template <typename T, size_t N> typename Bvh<T, N>::const_iterator Bvh<T, N>::itemOfNode(size_t i) const {
  if (isLeaf(i)) {
    return _nodes[i].item + begin();
  } else {
    return end();
...
template <typename T, size_t N>
size_t Bvh<T, N>::build(size_t nodeIndex, size_t *itemIndices, size_t nItems, size_t currentDepth) {
  // add a node
  _nodes.append(Node());
...
template <typename T, size_t N>
size_t Bvh<T, N>::qsplit(size_t *itemIndices, size_t numItems, double pivot, uint8_t axis) {
  double centroid;
  size_t ret = 0;
