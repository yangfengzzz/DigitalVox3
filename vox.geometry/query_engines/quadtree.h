...
//! \tparam     T     Value type.
//!
template <typename T> class Quadtree final : public IntersectionQueryEngine2<T>, public NearestNeighborQueryEngine2<T> {
public:
  ...
  void build(const std::vector<T> &items, const BoundingBox2D &bound, const BoxIntersectionTestFunc2<T> &testFunc,
             size_t maxDepth);
  ...
  //! function.
  NearestNeighborQueryResult2<T> nearest(const Vector2D &pt,
...
private:
  ...
  void build(size_t nodeIdx, size_t currentDepth, const BoundingBox2D &currentBound,
             const BoxIntersectionTestFunc2<T> &overlapsFunc);

  bool intersects(const BoundingBox2D &box, const BoxIntersectionTestFunc2<T> &testFunc, size_t nodeIdx,
                  const BoundingBox2D &currentBound) const;

  bool intersects(const Ray2D &ray, const RayIntersectionTestFunc2<T> &testFunc, size_t nodeIdx,
                  const BoundingBox2D &currentBound) const;
