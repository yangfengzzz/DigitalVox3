...
template <typename T, size_t N> Ray<T, N>::Ray(const Ray &other) : origin(other.origin), direction(other.direction) {}

template <typename T, size_t N> Ray<T, N> &Ray<T, N>::operator=(const Ray &other) {
  origin = other.origin;
...
template <typename T, size_t N> typename Ray<T, N>::VectorType Ray<T, N>::pointAt(T t) const {
