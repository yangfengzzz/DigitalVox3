...
//! collocated.
//!
template <size_t N> class FaceCenteredGrid final : public VectorGrid<N> {
public:
  ...
  explicit FaceCenteredGrid(const Vector<size_t, N> &resolution,
  ...
  //! Returns u-value at given data point.
  template <typename... Indices> double &u(size_t i, Indices... indices) { return u(Vector<size_t, N>(i, indices...)); }

  //! Returns u-value at given data point.
  const double &u(const Vector<size_t, N> &idx) const;

  //! Returns u-value at given data point.
  template <typename... Indices> const double &u(size_t i, Indices... indices) const {
  ...
  //! Returns v-value at given data point.
  template <typename... Indices> double &v(size_t i, Indices... indices) { return v(Vector<size_t, N>(i, indices...)); }

  //! Returns v-value at given data point.
  const double &v(const Vector<size_t, N> &idx) const;

  //! Returns v-value at given data point.
  template <typename... Indices> const double &v(size_t i, Indices... indices) const {
    return v(Vector<size_t, N>(i, indices...));
  }

  //! Returns w-value at given data point.
  template <size_t M = N> std::enable_if_t<M == 3, double &> w(const Vector<size_t, N> &idx) { return _data[2](idx); }

  //! Returns w-value at given data point.
  template <size_t M = N, typename... Indices> std::enable_if_t<M == 3, double &> w(size_t i, Indices... indices) {
    return w(Vector<size_t, N>(i, indices...));
  }

  //! Returns w-value at given data point.
  template <size_t M = N> std::enable_if_t<M == 3, const double &> w(const Vector<size_t, N> &idx) const {
    return _data[2](idx);
  }
  ...
  //! Returns interpolated value at cell center.
  template <typename... Indices> Vector<double, N> valueAtCellCenter(size_t i, Indices... indices) const {
  ...
  //! Returns divergence at cell-center location.
  template <typename... Indices> double divergenceAtCellCenter(size_t i, Indices... indices) const {
    return divergenceAtCellCenter(Vector<size_t, N>(i, indices...));
  }

  //! Returns curl at cell-center location.
  typename GetCurl<N>::type curlAtCellCenter(const Vector<size_t, N> &idx) const;

  //! Returns curl at cell-center location.
  template <typename... Indices> typename GetCurl<N>::type curlAtCellCenter(size_t i, Indices... indices) const {
  ...
  //! Returns w data view.
  template <size_t M = N> std::enable_if_t<M == 3, ScalarDataView> wView() { return dataView(2); }

  //! Returns read-only w data view.
  template <size_t M = N> std::enable_if_t<M == 3, ConstScalarDataView> wView() const { return dataView(2); }
  ...
  template <size_t M = N> std::enable_if_t<M == 3, GridDataPositionFunc<N>> wPosition() const {
  ...
  template <size_t M = N> std::enable_if_t<M == 3, Vector<size_t, N>> wSize() const { return dataSize(2); }
  ...
  //! the lower corner point of the bounding box.
  //!
  template <size_t M = N> std::enable_if_t<M == 3, Vector<double, N>> wOrigin() const { return dataOrigin(2); }
  ...
  void fill(const Vector<double, N> &value, ExecutionPolicy policy = ExecutionPolicy::kParallel) override;

  //! Fills the grid with given function.
  void fill(const std::function<Vector<double, N>(const Vector<double, N> &)> &func,
  ...
  parallelForEachWIndex(const std::function<void(const Vector<size_t, N> &)> &func) const {
...
protected:
  ...
  void onResize(const Vector<size_t, N> &resolution, const Vector<double, N> &gridSpacing,
                const Vector<double, N> &origin, const Vector<double, N> &initialValue) final;
...
//!
template <size_t N> class FaceCenteredGrid<N>::Builder final : public VectorGridBuilder<N> {
public:
  ...
  //!
  std::shared_ptr<VectorGrid<N>> build(const Vector<size_t, N> &resolution, const Vector<double, N> &gridSpacing,
