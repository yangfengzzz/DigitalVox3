...
public:
  ...
  void resize(const Vector<size_t, N> &resolution,
              ...
              const Vector<double, N> &origin = Vector<double, N>(), double initialValue = 0.0);
  ...
  //! Returns the grid data at given data point.
  template <typename... Indices> const double &operator()(size_t i, Indices... indices) const {
    return (*this)(Vector<size_t, N>(i, indices...));
  }

  //! Returns the grid data at given data point.
  template <typename... Indices> double &operator()(size_t i, Indices... indices) {
    return (*this)(Vector<size_t, N>(i, indices...));
  }
  ...
  //! Returns the gradient vector at given data point.
  template <typename... Indices> Vector<double, N> gradientAtDataPoint(size_t i, Indices... indices) const {
  ...
  //! Returns the Laplacian at given data point.
  template <typename... Indices> double laplacianAtDataPoint(size_t i, Indices... indices) const {
...
template <size_t N> class ScalarGridBuilder {
public:
  //! Creates a builder.
  ...
  virtual std::shared_ptr<ScalarGrid<N>> build(const Vector<size_t, N> &resolution,
