...
public:
  ...
  virtual void fill(const Vector<double, N> &value, ExecutionPolicy policy = ExecutionPolicy::kParallel) = 0;

  //! Fills the grid with given position-to-value mapping function.
  virtual void fill(const std::function<Vector<double, N>(const Vector<double, N> &)> &func,
...
protected:
  ...
  //! data layout scheme.
  //!
  virtual void onResize(const Vector<size_t, N> &resolution, const Vector<double, N> &gridSpacing,
                        const Vector<double, N> &origin, const Vector<double, N> &initialValue) = 0;
};
...
template <size_t N> class VectorGridBuilder {
public:
  //! Creates a builder.
  ...
  virtual std::shared_ptr<VectorGrid<N>> build(const Vector<size_t, N> &resolution,
