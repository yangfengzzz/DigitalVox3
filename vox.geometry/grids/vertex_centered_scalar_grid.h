...
//! (B+1) x ... data points.
//!
template <size_t N> class VertexCenteredScalarGrid final : public ScalarGrid<N> {
public:
  ...
  //! initial value.
  explicit VertexCenteredScalarGrid(const Vector<size_t, N> &resolution,
  ...
  //! Sets the contents with the given \p other grid.
  VertexCenteredScalarGrid<N> &operator=(const VertexCenteredScalarGrid<N> &other);
...
//!
template <size_t N> class VertexCenteredScalarGrid<N>::Builder final : public ScalarGridBuilder<N> {
public:
  ...
  //!
  std::shared_ptr<ScalarGrid<N>> build(const Vector<size_t, N> &resolution, const Vector<double, N> &gridSpacing,
