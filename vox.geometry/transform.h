...
public:
  Orientation();
  explicit Orientation(double angleInRadian);
...
public:
  Orientation();
  explicit Orientation(const QuaternionD &quat);

  [[nodiscard]] const QuaternionD &rotation() const;
...
//!
template <size_t N> class Transform {
public:
