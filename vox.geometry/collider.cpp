...
void Collider<N>::resolveCollision(double radius, double restitutionCoefficient, Vector<double, N> *newPosition,
  ...
  if (isPenetrating(colliderPoint, *newPosition, radius)) {
    ...
    Vector<double, N> targetPoint = colliderPoint.point + radius * targetNormal;
    ...
    if (normalDotRelativeVel < 0.0) {
      ...
      // the velocity
      Vector<double, N> deltaRelativeVelN = (-restitutionCoefficient - 1.0) * relativeVelN;
      ...
      if (relativeVelT.lengthSquared() > 0.0) {
        double frictionScale =
            std::max(1.0 - _frictionCoefficient * deltaRelativeVelN.length() / relativeVelT.length(), 0.0);
...
template <size_t N> double Collider<N>::frictionCoefficient() const { return _frictionCoefficient; }

template <size_t N> void Collider<N>::setFrictionCoefficient(double newFrictionCoefficient) {
  _frictionCoefficient = std::max(newFrictionCoefficient, 0.0);
}

template <size_t N> const std::shared_ptr<Surface<N>> &Collider<N>::surface() const { return _surface; }

template <size_t N> void Collider<N>::setSurface(const std::shared_ptr<Surface<N>> &newSurface) {
  _surface = newSurface;
}

template <size_t N>
void Collider<N>::getClosestPoint(const std::shared_ptr<Surface<N>> &surface, const Vector<double, N> &queryPoint,
...
template <size_t N>
bool Collider<N>::isPenetrating(const ColliderQueryResult &colliderPoint, const Vector<double, N> &position,
                                double radius) {
...
template <size_t N> void Collider<N>::update(double currentTimeInSeconds, double timeIntervalInSeconds) {
  JET_ASSERT(_surface);
...
template <size_t N> void Collider<N>::setOnBeginUpdateCallback(const OnBeginUpdateCallback &callback) {
