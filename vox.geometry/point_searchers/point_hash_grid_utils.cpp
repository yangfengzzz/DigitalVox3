...
template <size_t N>
template <size_t M>
std::enable_if_t<M == 3, size_t> PointHashGridUtils<N>::hashKey(const Vector<ssize_t, 3> &index,
                                                                const Vector<ssize_t, 3> &resolution) {
  return static_cast<size_t>((index.z * resolution.y + index.y) * resolution.x + index.x);
}

template <size_t N>
size_t PointHashGridUtils<N>::getHashKeyFromBucketIndex(const Vector<ssize_t, N> &bucketIndex,
...
template <size_t N>
Vector<ssize_t, N> PointHashGridUtils<N>::getBucketIndex(const Vector<double, N> &position, double gridSpacing) {
...
template <size_t N>
size_t PointHashGridUtils<N>::getHashKeyFromPosition(const Vector<double, N> &position, double gridSpacing,
                                                     const Vector<ssize_t, N> &resolution) {
...
template <size_t N>
void PointHashGridUtils<N>::getNearbyKeys(const Vector<double, N> &position, double gridSpacing,
                                          const Vector<ssize_t, N> &resolution, size_t *nearbyKeys) {
  ...
  for (size_t axis = 0; axis < N; axis++) {
    int offset = (originIndex[axis] + 0.5) * gridSpacing <= position[axis] ? 1 : -1;
  ...
  for (int i = 0; i < kNumKeys; i++) {
    nearbyKeys[i] = getHashKeyFromBucketIndex(nearbyBucketIndices[i], resolution);
  }
}
