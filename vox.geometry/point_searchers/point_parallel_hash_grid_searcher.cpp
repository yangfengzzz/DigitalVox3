...
PointParallelHashGridSearcher<N>::PointParallelHashGridSearcher(const Vector<size_t, N> &resolution, double gridSpacing)
  ...
  _resolution = max(resolution.template castTo<ssize_t>(), Vector<ssize_t, N>::makeConstant(kOneSSize));
  auto tableSize = static_cast<size_t>(product(_resolution, kOneSSize));
  _startIndexTable.resize(tableSize, kMaxSize);
  _endIndexTable.resize(tableSize, kMaxSize);
}

template <size_t N>
PointParallelHashGridSearcher<N>::PointParallelHashGridSearcher(const PointParallelHashGridSearcher &other) {
  set(other);
}

template <size_t N>
void PointParallelHashGridSearcher<N>::build(const ConstArrayView1<Vector<double, N>> &points, double maxSearchRadius) {
  ...
  parallelFor(kZeroSize, numberOfPoints, [&](size_t i) {
    _sortedIndices[i] = i;
    _points[i] = points[i];
    tempKeys[i] = PointHashGridUtils<N>::getHashKeyFromPosition(points[i], _gridSpacing, _resolution);
  });
...
template <size_t N>
void PointParallelHashGridSearcher<N>::forEachNearbyPoint(const Vector<double, N> &origin, double radius,
                                                          const ForEachNearbyPointFunc &callback) {
  constexpr int kNumKeys = 1 << N;
  size_t nearbyKeys[kNumKeys];
  PointHashGridUtils<N>::getNearbyKeys(origin, _gridSpacing, _resolution, nearbyKeys);
...
template <size_t N>
bool PointParallelHashGridSearcher<N>::hasNearbyPoint(const Vector<double, N> &origin, double radius) {
  constexpr int kNumKeys = 1 << N;
  size_t nearbyKeys[kNumKeys];
  PointHashGridUtils<N>::getNearbyKeys(origin, _gridSpacing, _resolution, nearbyKeys);
...
template <size_t N> ConstArrayView1<size_t> PointParallelHashGridSearcher<N>::keys() const { return _keys; }

template <size_t N> ConstArrayView1<size_t> PointParallelHashGridSearcher<N>::startIndexTable() const {
  return _startIndexTable;
}

template <size_t N> ConstArrayView1<size_t> PointParallelHashGridSearcher<N>::endIndexTable() const {
  return _endIndexTable;
}

template <size_t N> ConstArrayView1<size_t> PointParallelHashGridSearcher<N>::sortedIndices() const {
  return _sortedIndices;
}

template <size_t N> std::shared_ptr<PointNeighborSearcher<N>> PointParallelHashGridSearcher<N>::clone() const {
  return CLONE_W_CUSTOM_DELETER(PointParallelHashGridSearcher);
}

template <size_t N>
PointParallelHashGridSearcher<N> &
PointParallelHashGridSearcher<N>::operator=(const PointParallelHashGridSearcher &other) {
  set(other);
  return *this;
}

template <size_t N> void PointParallelHashGridSearcher<N>::set(const PointParallelHashGridSearcher &other) {
...
template <size_t N> void PointParallelHashGridSearcher<N>::serialize(std::vector<uint8_t> *buffer) const {
  serialize(*this, buffer);
}

template <size_t N> void PointParallelHashGridSearcher<N>::deserialize(const std::vector<uint8_t> &buffer) {
  deserialize(buffer, *this);
}

template <size_t N> typename PointParallelHashGridSearcher<N>::Builder PointParallelHashGridSearcher<N>::builder() {
  return Builder();
}
...
PointParallelHashGridSearcher<N>::serialize(const PointParallelHashGridSearcher<2> &searcher,
                                            std::vector<uint8_t> *buffer) {
  flatbuffers::FlatBufferBuilder builder(1024);

  // Copy simple data
  auto fbsResolution = fbs::Vector2UZ(searcher._resolution.x, searcher._resolution.y);
  ...
  std::vector<uint64_t> startIndexTable(searcher._startIndexTable.begin(), searcher._startIndexTable.end());
  std::vector<uint64_t> endIndexTable(searcher._endIndexTable.begin(), searcher._endIndexTable.end());
  std::vector<uint64_t> sortedIndices(searcher._sortedIndices.begin(), searcher._sortedIndices.end());
  ...
  // Copy the searcher
  auto fbsSearcher =
      fbs::CreatePointParallelHashGridSearcher2(builder, searcher._gridSpacing, &fbsResolution, fbsPoints, fbsKeys,
...
PointParallelHashGridSearcher<N>::serialize(const PointParallelHashGridSearcher<3> &searcher,
                                            std::vector<uint8_t> *buffer) {
  flatbuffers::FlatBufferBuilder builder(1024);

  // Copy simple data
  auto fbsResolution = fbs::Vector3UZ(searcher._resolution.x, searcher._resolution.y, searcher._resolution.z);
  ...
  // Copy the searcher
  auto fbsSearcher =
      fbs::CreatePointParallelHashGridSearcher3(builder, searcher._gridSpacing, &fbsResolution, fbsPoints, fbsKeys,
                                                fbsStartIndexTable, fbsEndIndexTable, fbsSortedIndices);
...
template <size_t M>
std::enable_if_t<M == 2, void>
PointParallelHashGridSearcher<N>::deserialize(const std::vector<uint8_t> &buffer,
  ...
  for (uint32_t i = 0; i < fbsStartIndexTable->size(); ++i) {
    searcher._startIndexTable[i] = static_cast<size_t>(fbsStartIndexTable->Get(i));
  }
  ...
  for (uint32_t i = 0; i < fbsEndIndexTable->size(); ++i) {
    searcher._endIndexTable[i] = static_cast<size_t>(fbsEndIndexTable->Get(i));
  }
  ...
  for (uint32_t i = 0; i < fbsSortedIndices->size(); ++i) {
    searcher._sortedIndices[i] = static_cast<size_t>(fbsSortedIndices->Get(i));
  }
}
...
PointParallelHashGridSearcher<N>::Builder::withResolution(const Vector<size_t, N> &resolution) {
...
template <size_t N> PointParallelHashGridSearcher<N> PointParallelHashGridSearcher<N>::Builder::build() const {
  return PointParallelHashGridSearcher(_resolution, _gridSpacing);
}

template <size_t N>
std::shared_ptr<PointParallelHashGridSearcher<N>> PointParallelHashGridSearcher<N>::Builder::makeShared() const {
  return std::shared_ptr<PointParallelHashGridSearcher>(new PointParallelHashGridSearcher(_resolution, _gridSpacing),
