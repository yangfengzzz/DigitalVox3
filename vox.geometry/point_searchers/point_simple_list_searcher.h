...
//!
template <size_t N> class PointSimpleListSearcher final : public PointNeighborSearcher<N> {
public:
  ...
  //!
  //! \brief      Builds internal structure for given points list and max search
  //! radius.
  //!
  ...
  //!
  void forEachNearbyPoint(const Vector<double, N> &origin, double radius,
  ...
  //!
  bool hasNearbyPoint(const Vector<double, N> &origin, double radius) override;
...
private:
  ...
  template <size_t M = N>
  static std::enable_if_t<M == 2, void> serialize(const PointSimpleListSearcher<2> &searcher,
                                                  std::vector<uint8_t> *buffer);

  template <size_t M = N>
  static std::enable_if_t<M == 3, void> serialize(const PointSimpleListSearcher<3> &searcher,
                                                  std::vector<uint8_t> *buffer);

  template <size_t M = N>
  static std::enable_if_t<M == 2, void> deserialize(const std::vector<uint8_t> &buffer,
                                                    PointSimpleListSearcher<2> &searcher);

  template <size_t M = N>
  static std::enable_if_t<M == 3, void> deserialize(const std::vector<uint8_t> &buffer,
...
//!
template <size_t N> class PointSimpleListSearcher<N>::Builder final : public PointNeighborSearcherBuilder<N> {
public:
