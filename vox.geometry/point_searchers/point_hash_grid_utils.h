...
//!
template <size_t N> class PointHashGridUtils {
public:
  template <size_t M = N>
  static std::enable_if_t<M == 2, size_t> hashKey(const Vector<ssize_t, 2> &index,
                                                  const Vector<ssize_t, 2> &resolution);

  template <size_t M = N>
  static std::enable_if_t<M == 3, size_t> hashKey(const Vector<ssize_t, 3> &index,
  ...
  static size_t getHashKeyFromPosition(const Vector<double, N> &position, double gridSpacing,
                                       const Vector<ssize_t, N> &resolution);

  static void getNearbyKeys(const Vector<double, N> &position, double gridSpacing, const Vector<ssize_t, N> &resolution,
                            size_t *nearbyKeys);
};
