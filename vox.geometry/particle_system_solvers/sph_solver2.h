...
public:
  ...
  [[nodiscard]] double eosExponent() const;
  ...
  [[nodiscard]] double negativePressureScale() const;
  ...
  //!
  [[nodiscard]] double timeStepLimitScale() const;
  ...
  [[nodiscard]] SphSystemData2Ptr sphSystemData() const;
...
protected:
  ...
  [[nodiscard]] unsigned int numberOfSubTimeSteps(double timeIntervalInSeconds) const override;
  ...
  void accumulatePressureForce(const ConstArrayView1<Vector2D> &positions, const ConstArrayView1<double> &densities,
                               const ConstArrayView1<double> &pressures, ArrayView1<Vector2D> pressureForces) const;
...
//!
template <typename DerivedBuilder> class SphSolverBuilderBase2 {
public:
...
template <typename T> T &SphSolverBuilderBase2<T>::withTargetDensity(double targetDensity) {
  _targetDensity = targetDensity;
  return static_cast<T &>(*this);
}

template <typename T> T &SphSolverBuilderBase2<T>::withTargetSpacing(double targetSpacing) {
  _targetSpacing = targetSpacing;
  return static_cast<T &>(*this);
}

template <typename T> T &SphSolverBuilderBase2<T>::withRelativeKernelRadius(double relativeKernelRadius) {
