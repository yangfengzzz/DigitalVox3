...
public:
  ...
  [[nodiscard]] SphSystemData3Ptr sphSystemData() const;
...
protected:
  ...
  void accumulatePressureForce(const ConstArrayView1<Vector3D> &positions, const ConstArrayView1<double> &densities,
                               const ConstArrayView1<double> &pressures, ArrayView1<Vector3D> pressureForces) const;
...
//!
template <typename DerivedBuilder> class SphSolverBuilderBase3 {
public:
...
template <typename T> T &SphSolverBuilderBase3<T>::withTargetDensity(double targetDensity) {
  _targetDensity = targetDensity;
  return static_cast<T &>(*this);
}

template <typename T> T &SphSolverBuilderBase3<T>::withTargetSpacing(double targetSpacing) {
  _targetSpacing = targetSpacing;
  return static_cast<T &>(*this);
}

template <typename T> T &SphSolverBuilderBase3<T>::withRelativeKernelRadius(double relativeKernelRadius) {
