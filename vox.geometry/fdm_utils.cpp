...
std::array<Vector2D, 2> gradient2(const ConstArrayView2<Vector2D> &data, const Vector2D &gridSpacing, size_t i,
                                  size_t j) {
  const Vector2UZ &ds = data.size();
  ...
  result[0] = 0.5 * elemDiv(Vector2D(right.x - left.x, up.x - down.x), gridSpacing);
  result[1] = 0.5 * elemDiv(Vector2D(right.y - left.y, up.y - down.y), gridSpacing);
  return result;
}
...
Vector3D gradient3(const ConstArrayView3<double> &data, const Vector3D &gridSpacing, size_t i, size_t j, size_t k) {
  ...
  return 0.5 * elemDiv(Vector3D(right - left, up - down, front - back), gridSpacing);
}

std::array<Vector3D, 3> gradient3(const ConstArrayView3<Vector3D> &data, const Vector3D &gridSpacing, size_t i,
                                  size_t j, size_t k) {
  const Vector3UZ &ds = data.size();
  ...
  result[0] = 0.5 * elemDiv(Vector3D(right.x - left.x, up.x - down.x, front.x - back.x), gridSpacing);
  result[1] = 0.5 * elemDiv(Vector3D(right.y - left.y, up.y - down.y, front.y - back.y), gridSpacing);
  result[2] = 0.5 * elemDiv(Vector3D(right.z - left.z, up.z - down.z, front.z - back.z), gridSpacing);
  return result;
}
...
double divergence2(const ConstArrayView2<Vector2D> &data, const Vector2D &gridSpacing, size_t i, size_t j) {
  ...
  return 0.5 * (right - left) / gridSpacing.x + 0.5 * (up - down) / gridSpacing.y;
}
...
double curl2(const ConstArrayView2<Vector2D> &data, const Vector2D &gridSpacing, size_t i, size_t j) {
  ...
  return 0.5 * (Fy_xp - Fy_xm) / gridSpacing.x - 0.5 * (Fx_yp - Fx_ym) / gridSpacing.y;
}
...
Vector3D curl3(const ConstArrayView3<Vector3D> &data, const Vector3D &gridSpacing, size_t i, size_t j, size_t k) {
  ...
  return Vector3D(0.5 * (Fz_yp - Fz_ym) / gridSpacing.y - 0.5 * (Fy_zp - Fy_zm) / gridSpacing.z,
