...
template <typename T, size_t N> void fill(ArrayView<T, N> a, const T &val) {
  fill(a, Vector<size_t, N>{}, Vector<size_t, N>{a.size()}, val);
}

template <typename T> void fill(ArrayView<T, 1> a, size_t begin, size_t end, const T &val) {
...
template <typename T, typename U, size_t N> void copy(ArrayView<T, N> src, ArrayView<U, N> dst) {
  copy(src, Vector<size_t, N>{}, Vector<size_t, N>{src.size()}, dst);
}

template <typename T, typename U> void copy(ArrayView<T, 1> src, size_t begin, size_t end, ArrayView<U, 1> dst) {
  copy(src, Vector1UZ{begin}, Vector1UZ{end}, dst);
}

template <typename T, typename U>
void extrapolateToRegion(ArrayView2<T> input, ArrayView2<char> valid, unsigned int numberOfIterations,
                         ArrayView2<U> output) {
  ...
  for (unsigned int iter = 0; iter < numberOfIterations; ++iter) {
    forEachIndex(valid0.size(), [&](size_t i, size_t j) {
      ...
      if (!valid0(i, j)) {
        ...
        if (count > 0) {
          output(i, j) = sum / static_cast<typename GetScalarType<T>::value>(count);
          valid1(i, j) = 1;
        }
      } else {
        valid1(i, j) = 1;
  ...
  for (unsigned int iter = 0; iter < numberOfIterations; ++iter) {
    forEachIndex(valid0.size(), [&](size_t i, size_t j, size_t k) {
      ...
      if (!valid0(i, j, k)) {
        ...
        if (count > 0) {
          output(i, j, k) = sum / static_cast<typename GetScalarType<T>::value>(count);
          valid1(i, j, k) = 1;
        }
      } else {
        valid1(i, j, k) = 1;
