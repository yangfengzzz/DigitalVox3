...
public:
  ...
  template <size_t R, size_t C, typename E> void copyFrom(const MatrixExpression<T, R, C, E> &expression);
  ...
  template <size_t R, size_t C, typename E> void setRow(size_t i, const MatrixExpression<T, R, C, E> &row);

  //! Sets i-th column with input vector.
  template <size_t R, size_t C, typename E> void setColumn(size_t i, const MatrixExpression<T, R, C, E> &col);
  ...
  //! Copies from generic expression
  template <size_t R, size_t C, typename E> MatrixDenseBase &operator=(const MatrixExpression<T, R, C, E> &expression);

  // MARK: Builders

  //! Makes a static matrix with zero entries.
  template <typename D = Derived> static std::enable_if_t<isMatrixSizeStatic<Rows, Cols>(), D> makeZero();
  ...
  template <typename D = Derived>
  static std::enable_if_t<isMatrixSizeStatic<Rows, Cols>(), D> makeConstant(value_type val);

  //! Makes a dynamic matrix with constant entries.
  template <typename D = Derived>
  static std::enable_if_t<isMatrixSizeDynamic<Rows, Cols>(), D> makeConstant(size_t rows, size_t cols, value_type val);

  //! Makes a static identity matrix.
  template <typename D = Derived> static std::enable_if_t<isMatrixStaticSquare<Rows, Cols>(), D> makeIdentity();
  ...
  static std::enable_if_t<isMatrixStaticSquare<Rows, Cols>(), D> makeScaleMatrix(value_type first, Args... rest);
  ...
  //! \warning Input angle should be radian.
  template <size_t R, size_t C, typename E, typename D = Derived>
  static std::enable_if_t<isMatrixStaticSquare<Rows, Cols>() && (Rows == 3 || Rows == 4), D>
  makeRotationMatrix(const MatrixExpression<T, R, C, E> &axis, T rad);

  //! Makes translation matrix.
  template <size_t R, size_t C, typename E, typename D = Derived>
  static std::enable_if_t<isMatrixStaticSquare<Rows, Cols>() && (Rows == 4), D>
