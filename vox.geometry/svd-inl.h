...
template <typename T> inline T sign(T a, T b) { return b >= 0.0 ? std::fabs(a) : -std::fabs(a); }
...
template <typename T> void svd(const MatrixMxN<T> &a, MatrixMxN<T> &u, VectorN<T> &w, MatrixMxN<T> &v) {
  ...
  JET_THROW_INVALID_ARG_WITH_MESSAGE_IF(m < n, "Number of rows of input matrix must "
...
void svd(const Matrix<T, M, N> &a, Matrix<T, M, N> &u, Vector<T, N> &w, Matrix<T, N, N> &v) {
  ...
  static_assert(M >= N, "Number of rows of input matrix must be greater than or "
                        "equal to columns.");
