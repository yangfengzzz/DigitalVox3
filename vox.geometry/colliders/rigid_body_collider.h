...
public:
  ...
  explicit AngularVelocity(double val) : value(val) {}

  [[nodiscard]] Vector2D cross(const Vector2D &r) const { return value * Vector2D(-r.y, r.x); }
};
...
public:
  ...
  AngularVelocity() = default;
  explicit AngularVelocity(const Vector3D &val) : value(val) {}

  [[nodiscard]] Vector3D cross(const Vector3D &r) const { return value.cross(r); }
};
...
//!
template <size_t N> class RigidBodyCollider final : public Collider<N> {
public:
  class Builder;
  ...
  RigidBodyCollider(const std::shared_ptr<Surface<N>> &surface, const Vector<double, N> &linearVelocity,
...
//!
template <size_t N> class RigidBodyCollider<N>::Builder final {
public:
