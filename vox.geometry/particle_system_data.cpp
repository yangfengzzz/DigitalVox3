...
#include "point_searchers/point_parallel_hash_grid_searcher.h"
#include "timer.h"
...
template <size_t N> struct GetFlatbuffersParticleSystemData {};

template <> struct GetFlatbuffersParticleSystemData<2> {
  using Offset = flatbuffers::Offset<fbs::ParticleSystemData2>;

  static const fbs::ParticleSystemData2 *getParticleSystemData(const uint8_t *data) {
    return fbs::GetParticleSystemData2(data);
  }
};

template <> struct GetFlatbuffersParticleSystemData<3> {
  using Offset = flatbuffers::Offset<fbs::ParticleSystemData3>;

  static const fbs::ParticleSystemData3 *getParticleSystemData(const uint8_t *data) {
...
template <size_t N> ParticleSystemData<N>::ParticleSystemData() : ParticleSystemData(0) {}

template <size_t N> ParticleSystemData<N>::ParticleSystemData(size_t numberOfParticles) {
  ...
  _neighborSearcher = std::make_shared<PointParallelHashGridSearcher<N>>(
      Vector<size_t, N>::makeConstant(kDefaultHashGridResolution), 2.0 * _radius);

  resize(numberOfParticles);
}

template <size_t N> ParticleSystemData<N>::ParticleSystemData(const ParticleSystemData &other) { set(other); }

template <size_t N> void ParticleSystemData<N>::resize(size_t newNumberOfParticles) {
...
template <size_t N> size_t ParticleSystemData<N>::numberOfParticles() const { return _numberOfParticles; }

template <size_t N> size_t ParticleSystemData<N>::addScalarData(double initialVal) {
  size_t attrIdx = _scalarDataList.length();
  _scalarDataList.append(ScalarData(numberOfParticles(), initialVal));
  return attrIdx;
}

template <size_t N> size_t ParticleSystemData<N>::addVectorData(const Vector<double, N> &initialVal) {
  size_t attrIdx = _vectorDataList.length();
  _vectorDataList.append(VectorData(numberOfParticles(), initialVal));
  return attrIdx;
}

template <size_t N> double ParticleSystemData<N>::radius() const { return _radius; }

template <size_t N> void ParticleSystemData<N>::setRadius(double newRadius) { _radius = std::max(newRadius, 0.0); }

template <size_t N> double ParticleSystemData<N>::mass() const { return _mass; }

template <size_t N> void ParticleSystemData<N>::setMass(double newMass) { _mass = std::max(newMass, 0.0); }

template <size_t N> ConstArrayView1<Vector<double, N>> ParticleSystemData<N>::positions() const {
  return vectorDataAt(_positionIdx);
}

template <size_t N> ArrayView1<Vector<double, N>> ParticleSystemData<N>::positions() {
  return vectorDataAt(_positionIdx);
}

template <size_t N> ConstArrayView1<Vector<double, N>> ParticleSystemData<N>::velocities() const {
  return vectorDataAt(_velocityIdx);
}

template <size_t N> ArrayView1<Vector<double, N>> ParticleSystemData<N>::velocities() {
  return vectorDataAt(_velocityIdx);
}

template <size_t N> ConstArrayView1<Vector<double, N>> ParticleSystemData<N>::forces() const {
  return vectorDataAt(_forceIdx);
}

template <size_t N> ArrayView1<Vector<double, N>> ParticleSystemData<N>::forces() { return vectorDataAt(_forceIdx); }

template <size_t N> ConstArrayView1<double> ParticleSystemData<N>::scalarDataAt(size_t idx) const {
  return ConstArrayView1<double>(_scalarDataList[idx]);
}

template <size_t N> ArrayView1<double> ParticleSystemData<N>::scalarDataAt(size_t idx) {
  return ArrayView1<double>(_scalarDataList[idx]);
}

template <size_t N> ConstArrayView1<Vector<double, N>> ParticleSystemData<N>::vectorDataAt(size_t idx) const {
  return ConstArrayView1<Vector<double, N>>(_vectorDataList[idx]);
}

template <size_t N> ArrayView1<Vector<double, N>> ParticleSystemData<N>::vectorDataAt(size_t idx) {
  return ArrayView1<Vector<double, N>>(_vectorDataList[idx]);
}

template <size_t N>
void ParticleSystemData<N>::addParticle(const Vector<double, N> &newPosition, const Vector<double, N> &newVelocity,
...
template <size_t N>
void ParticleSystemData<N>::addParticles(const ConstArrayView1<Vector<double, N>> &newPositions,
                                         const ConstArrayView1<Vector<double, N>> &newVelocities,
                                         const ConstArrayView1<Vector<double, N>> &newForces) {
  JET_THROW_INVALID_ARG_IF(newVelocities.length() > 0 && newVelocities.length() != newPositions.length());
  JET_THROW_INVALID_ARG_IF(newForces.length() > 0 && newForces.length() != newPositions.length());
  ...
  parallelFor(kZeroSize, newPositions.length(), [&](size_t i) { pos[i + oldNumberOfParticles] = newPositions[i]; });

  if (newVelocities.length() > 0) {
    parallelFor(kZeroSize, newPositions.length(), [&](size_t i) { vel[i + oldNumberOfParticles] = newVelocities[i]; });
  }

  if (newForces.length() > 0) {
    parallelFor(kZeroSize, newPositions.length(), [&](size_t i) { frc[i + oldNumberOfParticles] = newForces[i]; });
  }
}

template <size_t N> const std::shared_ptr<PointNeighborSearcher<N>> &ParticleSystemData<N>::neighborSearcher() const {
  return _neighborSearcher;
}

template <size_t N>
void ParticleSystemData<N>::setNeighborSearcher(const std::shared_ptr<PointNeighborSearcher<N>> &newNeighborSearcher) {
  _neighborSearcher = newNeighborSearcher;
}

template <size_t N> const Array1<Array1<size_t>> &ParticleSystemData<N>::neighborLists() const {
  return _neighborLists;
}

template <size_t N> void ParticleSystemData<N>::buildNeighborSearcher(double maxSearchRadius) {
  Timer timer;
...
template <size_t N> void ParticleSystemData<N>::buildNeighborLists(double maxSearchRadius) {
  Timer timer;
  ...
  for (size_t i = 0; i < numberOfParticles(); ++i) {
    ...
    _neighborSearcher->forEachNearbyPoint(origin, maxSearchRadius, [&](size_t j, const Vector<double, N> &) {
      if (i != j) {
...
template <size_t N> void ParticleSystemData<N>::serialize(std::vector<uint8_t> *buffer) const {
...
template <size_t N> void ParticleSystemData<N>::deserialize(const std::vector<uint8_t> &buffer) {
  auto fbsParticleSystemData = GetFlatbuffersParticleSystemData<N>::getParticleSystemData(buffer.data());
  deserialize(fbsParticleSystemData, *this);
}

template <size_t N> void ParticleSystemData<N>::set(const ParticleSystemData &other) {
...
template <size_t N> ParticleSystemData<N> &ParticleSystemData<N>::operator=(const ParticleSystemData &other) {
  set(other);
  return *this;
}
...
template <size_t N>
template <size_t M>
std::enable_if_t<M == 2, void>
ParticleSystemData<N>::serialize(const ParticleSystemData<2> &particles, flatbuffers::FlatBufferBuilder *builder,
  ...
  for (const auto &scalarData : particles._scalarDataList) {
    auto fbsScalarData =
        fbs::CreateScalarParticleData2(*builder, builder->CreateVector(scalarData.data(), scalarData.length()));
  ...
  auto neighborSearcherType = builder->CreateString(particles._neighborSearcher->typeName());
  ...
  *fbsParticleSystemData = fbs::CreateParticleSystemData2(
      *builder, particles._radius, particles._mass, particles._positionIdx, particles._velocityIdx, particles._forceIdx,
      fbsScalarDataList, fbsVectorDataList, fbsNeighborSearcher, fbsNeighborLists);
}

template <size_t N>
template <size_t M>
std::enable_if_t<M == 3, void>
ParticleSystemData<N>::serialize(const ParticleSystemData<3> &particles, flatbuffers::FlatBufferBuilder *builder,
  ...
  for (const auto &scalarData : particles._scalarDataList) {
    auto fbsScalarData =
        fbs::CreateScalarParticleData3(*builder, builder->CreateVector(scalarData.data(), scalarData.length()));
    scalarDataList.push_back(fbsScalarData);
  }
  auto fbsScalarDataList = builder->CreateVector(scalarDataList);
...
template <size_t N>
template <size_t M>
std::enable_if_t<M == 2, void> ParticleSystemData<N>::deserialize(const fbs::ParticleSystemData2 *fbsParticleSystemData,
  ...
  particles._positionIdx = static_cast<size_t>(fbsParticleSystemData->positionIdx());
  particles._velocityIdx = static_cast<size_t>(fbsParticleSystemData->velocityIdx());
  particles._forceIdx = static_cast<size_t>(fbsParticleSystemData->forceIdx());
  ...
  particles._neighborSearcher = Factory::buildPointNeighborSearcher2(fbsNeighborSearcher->type()->c_str());
  ...
  for (uint32_t i = 0; i < fbsNeighborLists->size(); ++i) {
    ...
    std::transform(fbsNeighborList->data()->begin(), fbsNeighborList->data()->end(),
                   particles._neighborLists[i].begin(), [](uint64_t val) { return static_cast<size_t>(val); });
  }
}

template <size_t N>
template <size_t M>
std::enable_if_t<M == 3, void> ParticleSystemData<N>::deserialize(const fbs::ParticleSystemData3 *fbsParticleSystemData,
  ...
  // Copy neighbor searcher
  auto fbsNeighborSearcher = fbsParticleSystemData->neighborSearcher();
  particles._neighborSearcher = Factory::buildPointNeighborSearcher3(fbsNeighborSearcher->type()->c_str());
  std::vector<uint8_t> neighborSearcherSerialized(fbsNeighborSearcher->data()->begin(),
                                                  fbsNeighborSearcher->data()->end());
  particles._neighborSearcher->deserialize(neighborSearcherSerialized);
