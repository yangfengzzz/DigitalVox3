...
template <size_t N> class Surface {
public:
  ...
  explicit Surface(const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);
...
//!
template <size_t N, typename DerivedBuilder> class SurfaceBuilderBase {
public:
...
template <size_t N, typename T> T &SurfaceBuilderBase<N, T>::withIsNormalFlipped(bool isNormalFlipped) {
  _isNormalFlipped = isNormalFlipped;
  return static_cast<T &>(*this);
}

template <size_t N, typename T> T &SurfaceBuilderBase<N, T>::withTranslation(const Vector<double, N> &translation) {
  _transform.setTranslation(translation);
  return static_cast<T &>(*this);
}

template <size_t N, typename T> T &SurfaceBuilderBase<N, T>::withOrientation(const Orientation<N> &orientation) {
  _transform.setOrientation(orientation);
  return static_cast<T &>(*this);
}

template <size_t N, typename T> T &SurfaceBuilderBase<N, T>::withTransform(const Transform<N> &transform) {
  _transform = transform;
  return static_cast<T &>(*this);
}

template <typename T> using SurfaceBuilderBase2 = SurfaceBuilderBase<2, T>;
