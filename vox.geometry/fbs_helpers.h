...
inline fbs::Vector3UZ jetToFbs(const Vector3UZ &vec) { return fbs::Vector3UZ(vec.x, vec.y, vec.z); }
...
inline Vector2UZ fbsToJet(const fbs::Vector2UZ &vec) { return Vector2UZ({vec.x(), vec.y()}); }

inline Vector3UZ fbsToJet(const fbs::Vector3UZ &vec) { return Vector3UZ({vec.x(), vec.y(), vec.z()}); }
...
void serializeGrid(flatbuffers::FlatBufferBuilder *builder, const std::vector<GridType> &gridList, FbsFactoryFunc func,
