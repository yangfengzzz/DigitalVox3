...
ImplicitTriangleMesh3::ImplicitTriangleMesh3(const TriangleMesh3Ptr &mesh, size_t resolutionX, double margin,
  ...
  if (mesh->numberOfTriangles() > 0 && mesh->numberOfPoints() > 0) {
    ...
    const auto resolutionY =
        static_cast<size_t>(std::ceil(static_cast<double>(resolutionX) * box.height() / box.width()));
    const auto resolutionZ =
        static_cast<size_t>(std::ceil(static_cast<double>(resolutionX) * box.depth() / box.width()));

    double dx = box.width() / static_cast<double>(resolutionX);

    _grid = std::make_shared<VertexCenteredScalarGrid3>();
    _grid->resize({resolutionX, resolutionY, resolutionZ}, {dx, dx, dx}, box.lowerCorner);
    ...
    _customImplicitSurface = CustomImplicitSurface3::builder()
                                 .withSignedDistanceFunction([&](const Vector3D &) -> double { return kMaxD; })
                                 .makeShared();
  }
}
...
ImplicitTriangleMesh3Ptr ImplicitTriangleMesh3::Builder::makeShared() const {
  return std::shared_ptr<ImplicitTriangleMesh3>(
      new ImplicitTriangleMesh3(_mesh, _resolutionX, _margin, _transform, _isNormalFlipped),
