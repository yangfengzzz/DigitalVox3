...
//!
template <size_t N> class ImplicitSurfaceSet final : public ImplicitSurface<N> {
public:
  class Builder;
  ...
  explicit ImplicitSurfaceSet(ConstArrayView1<std::shared_ptr<ImplicitSurface<N>>> surfaces,
                              const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);

  //! Constructs an implicit surface set using list of other surfaces.
  explicit ImplicitSurfaceSet(ConstArrayView1<std::shared_ptr<Surface<N>>> surfaces,
                              const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);
...
//!
template <size_t N>
class ImplicitSurfaceSet<N>::Builder final : public SurfaceBuilderBase<N, typename ImplicitSurfaceSet<N>::Builder> {
public:
