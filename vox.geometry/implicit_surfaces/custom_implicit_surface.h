...
public:
  ...
  //!
  explicit CustomImplicitSurface(const std::function<double(const Vector<double, N> &)> &func,
                                 const BoundingBox<double, N> &domain = BoundingBox<double, N>(),
                                 double resolution = 1e-3, double rayMarchingResolution = 1e-6,
                                 unsigned int numberOfIterations = 5, const Transform<N> &transform = Transform<N>(),
