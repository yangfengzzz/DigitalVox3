...
public:
  ...
  explicit GridDataPositionFunc(const RawFunctionType &func) : _func(func) {}

  template <typename... Indices> Vector<double, N> operator()(size_t i, Indices... indices) const {
    return (*this)(Vector<size_t, N>(i, indices...));
  }

  Vector<double, N> operator()(const Vector<size_t, N> &idx) const { return _func(idx); }
...
//!
template <size_t N> class Grid : public Serializable {
public:
  ...
  std::enable_if_t<M == 3, void> forEachCellIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    forEachCellIndex([&func](const Vector3UZ &idx) { func(idx.x, idx.y, idx.z); });
  }
  ...
  std::enable_if_t<M == 2, void> parallelForEachCellIndex(const std::function<void(size_t, size_t)> &func) const {
    parallelForEachCellIndex([&func](const Vector2UZ &idx) { func(idx.x, idx.y); });
  }

  template <size_t M = N>
  std::enable_if_t<M == 3, void>
  parallelForEachCellIndex(const std::function<void(size_t, size_t, size_t)> &func) const {
    parallelForEachCellIndex([&func](const Vector3UZ &idx) { func(idx.x, idx.y, idx.z); });
  }
...
protected:
  ...
  //! origin.
  void setSizeParameters(const Vector<size_t, N> &resolution, const Vector<double, N> &gridSpacing,
...
private:
  ...
  BoundingBox<double, N> _boundingBox = (BoundingBox<double, N>(Vector<double, N>(), Vector<double, N>()));
};
