...
inline double solveQuadNearBoundary(const Array2<char> &markers, ArrayView2<double> output, const Vector2D &gridSpacing,
                                    const Vector2D &invGridSpacingSqr, double sign, size_t i, size_t j) {
...
inline double solveQuad(const Array2<char> &markers, ArrayView2<double> output, const Vector2D &gridSpacing,
...
void FmmLevelSetSolver2::reinitialize(const ScalarGrid2 &inputSdf, double maxDistance, ScalarGrid2 *outputSdf) {
  ...
  parallelForEachIndex(markers.size(), [&](size_t i, size_t j) { output(i, j) = inputSdf(i, j); });

  // Solve geometrically near the boundary
  forEachIndex(markers.size(), [&](size_t i, size_t j) {
    if (!isInsideSdf(output(i, j)) &&
        ((i > 0 && isInsideSdf(output(i - 1, j))) || (i + 1 < size.x && isInsideSdf(output(i + 1, j))) ||
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    auto compare = [&](const Vector2UZ &a, const Vector2UZ &b) { return output(a.x, a.y) > output(b.x, b.y); };

    // Enqueue initial candidates
    std::priority_queue<Vector2UZ, std::vector<Vector2UZ>, decltype(compare)> trial(compare);
    forEachIndex(markers.size(), [&](size_t i, size_t j) {
      if (markers(i, j) != kKnown &&
          ((i > 0 && markers(i - 1, j) == kKnown) || (i + 1 < size.x && markers(i + 1, j) == kKnown) ||
    ...
    // Flip the sign
    parallelForEachIndex(markers.size(), [&](size_t i, size_t j) { output(i, j) = -output(i, j); });
  }
}

void FmmLevelSetSolver2::extrapolate(const ScalarGrid2 &input, const ScalarField2 &sdf, double maxDistance,
                                     ScalarGrid2 *output) {
  JET_THROW_INVALID_ARG_IF(!input.hasSameShape(*output));

  Array2<double> sdfGrid(input.dataSize());
  auto pos = input.dataPosition();
  parallelForEachIndex(sdfGrid.size(), [&](size_t i, size_t j) { sdfGrid(i, j) = sdf.sample(pos(i, j)); });

  extrapolate(input.dataView(), sdfGrid, input.gridSpacing(), maxDistance, output->dataView());
}

void FmmLevelSetSolver2::extrapolate(const CollocatedVectorGrid2 &input, const ScalarField2 &sdf, double maxDistance,
...
void FmmLevelSetSolver2::extrapolate(const FaceCenteredGrid2 &input, const ScalarField2 &sdf, double maxDistance,
  ...
  input.parallelForEachUIndex([&](const Vector2UZ &idx) { sdfAtU(idx) = sdf.sample(uPos(idx)); });
  ...
  input.parallelForEachVIndex([&](const Vector2UZ &idx) { sdfAtV(idx) = sdf.sample(vPos(idx)); });

  extrapolate(v, sdfAtV, gridSpacing, maxDistance, output->vView());
}

void FmmLevelSetSolver2::extrapolate(const ConstArrayView2<double> &input, const ConstArrayView2<double> &sdf,
  ...
  auto compare = [&](const Vector2UZ &a, const Vector2UZ &b) { return sdf(a.x, a.y) > sdf(b.x, b.y); };
