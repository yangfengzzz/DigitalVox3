...
inline double solveQuadNearBoundary(const Array3<char> &markers, ConstArrayView3<double> output,
                                    const Vector3D &gridSpacing, const Vector3D &invGridSpacingSqr, double sign,
                                    size_t i, size_t j, size_t k) {
  UNUSED_VARIABLE(markers);
  UNUSED_VARIABLE(invGridSpacingSqr);

  Vector3UZ size = output.size();
...
inline double solveQuad(const Array3<char> &markers, ArrayView3<double> output, const Vector3D &gridSpacing,
                        const Vector3D &invGridSpacingSqr, size_t i, size_t j, size_t k) {
  Vector3UZ size = output.size();
...
void FmmLevelSetSolver3::reinitialize(const ScalarGrid3 &inputSdf, double maxDistance, ScalarGrid3 *outputSdf) {
  ...
  parallelForEachIndex(markers.size(), [&](size_t i, size_t j, size_t k) { output(i, j, k) = inputSdf(i, j, k); });

  // Solve geometrically near the boundary
  forEachIndex(markers.size(), [&](size_t i, size_t j, size_t k) {
    if (isInsideSdf(output(i, j, k)) &&
        ((i > 0 && !isInsideSdf(output(i - 1, j, k))) || (i + 1 < size.x && !isInsideSdf(output(i + 1, j, k))) ||
      ...
      output(i, j, k) =
          solveQuadNearBoundary(markers, inputSdf.dataView(), gridSpacing, invGridSpacingSqr, -1.0, i, j, k);
    }
  });
  forEachIndex(markers.size(), [&](size_t i, size_t j, size_t k) {
    if (!isInsideSdf(output(i, j, k)) &&
  ...
  for (int sign = 0; sign < 2; ++sign) {
    ...
    std::priority_queue<Vector3UZ, std::vector<Vector3UZ>, decltype(compare)> trial(compare);
    forEachIndex(markers.size(), [&](size_t i, size_t j, size_t k) {
      if (markers(i, j, k) != kKnown &&
          ((i > 0 && markers(i - 1, j, k) == kKnown) || (i + 1 < size.x && markers(i + 1, j, k) == kKnown) ||
    ...
    // Flip the sign
    parallelForEachIndex(markers.size(), [&](size_t i, size_t j, size_t k) { output(i, j, k) = -output(i, j, k); });
  }
}

void FmmLevelSetSolver3::extrapolate(const ScalarGrid3 &input, const ScalarField3 &sdf, double maxDistance,
                                     ScalarGrid3 *output) {
  ...
  parallelForEachIndex(sdfGrid.size(),
                       [&](size_t i, size_t j, size_t k) { sdfGrid(i, j, k) = sdf.sample(pos(i, j, k)); });

  extrapolate(input.dataView(), sdfGrid, input.gridSpacing(), maxDistance, output->dataView());
}

void FmmLevelSetSolver3::extrapolate(const CollocatedVectorGrid3 &input, const ScalarField3 &sdf, double maxDistance,
...
void FmmLevelSetSolver3::extrapolate(const FaceCenteredGrid3 &input, const ScalarField3 &sdf, double maxDistance,
  ...
  input.parallelForEachUIndex([&](const Vector3UZ &idx) { sdfAtU(idx) = sdf.sample(uPos(idx)); });
  ...
  input.parallelForEachVIndex([&](const Vector3UZ &idx) { sdfAtV(idx) = sdf.sample(vPos(idx)); });
  ...
  input.parallelForEachWIndex([&](const Vector3UZ &idx) { sdfAtW(idx) = sdf.sample(wPos(idx)); });

  extrapolate(w, sdfAtW, gridSpacing, maxDistance, output->wView());
}

void FmmLevelSetSolver3::extrapolate(const ConstArrayView3<double> &input, const ConstArrayView3<double> &sdf,
  ...
  auto compare = [&](const Vector3UZ &a, const Vector3UZ &b) { return sdf(a.x, a.y, a.z) > sdf(b.x, b.y, b.z); };
