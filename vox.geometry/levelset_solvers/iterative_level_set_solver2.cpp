...
void IterativeLevelSetSolver2::extrapolate(const ScalarGrid2 &input, const ScalarField2 &sdf, double maxDistance,
                                           ScalarGrid2 *output) {
...
void IterativeLevelSetSolver2::extrapolate(const CollocatedVectorGrid2 &input, const ScalarField2 &sdf,
...
void IterativeLevelSetSolver2::extrapolate(const FaceCenteredGrid2 &input, const ScalarField2 &sdf, double maxDistance,
...
void IterativeLevelSetSolver2::extrapolate(const ConstArrayView2<double> &input, const ConstArrayView2<double> &sdf,
  ...
  for (unsigned int n = 0; n < numberOfIterations; ++n) {
    parallelFor(kZeroSize, size.x, kZeroSize, size.y, [&](size_t i, size_t j) {
      if (sdf(i, j) >= 0) {
        ...
        tempAcc(i, j) = outputAcc(i, j) - dtau * (std::max(grad.x, 0.0) * dx[0] + std::min(grad.x, 0.0) * dx[1] +
...
double IterativeLevelSetSolver2::sign(const ConstArrayView2<double> &sdf, const Vector2D &gridSpacing, size_t i,
                                      size_t j) {
  double d = sdf(i, j);
...
double IterativeLevelSetSolver2::pseudoTimeStep(const ConstArrayView2<double> &sdf, const Vector2D &gridSpacing) const {
