...
template <size_t N>
Box<N>::Box(const Vector<double, N> &lowerCorner, const Vector<double, N> &upperCorner, const Transform<N> &transform,
            bool isNormalFlipped)
    : Box(BoundingBox<double, N>(lowerCorner, upperCorner), transform, isNormalFlipped) {}

template <size_t N>
Box<N>::Box(const BoundingBox<double, N> &boundingBox, const Transform<N> &transform, bool isNormalFlipped)
    : Surface<N>(transform, isNormalFlipped), bound(boundingBox) {}

template <size_t N> Box<N>::Box(const Box &other) : Surface<N>(other), bound(other.bound) {}

template <size_t N> Vector<double, N> Box<N>::closestPointLocal(const Vector<double, N> &otherPoint) const {
...
template <size_t N> Vector<double, N> Box<N>::closestNormalLocal(const Vector<double, N> &otherPoint) const {
  Plane<N> planes[2 * N];
  for (size_t i = 0; i < N; ++i) {
    Vector<double, N> normal;
    normal[i] = 1.0;
  ...
  if (bound.contains(otherPoint)) {
    ...
    for (size_t i = 1; i < 2 * N; ++i) {
      Vector<double, N> localClosestPoint = planes[i].closestPoint(otherPoint);
    ...
    Vector<double, N> closestPoint = clamp(otherPoint, bound.lowerCorner, bound.upperCorner);
...
template <size_t N> bool Box<N>::intersectsLocal(const Ray<double, N> &ray) const { return bound.intersects(ray); }

template <size_t N> SurfaceRayIntersection<N> Box<N>::closestIntersectionLocal(const Ray<double, N> &ray) const {
  SurfaceRayIntersection<N> intersection;
  BoundingBoxRayIntersectionD bbRayIntersection = bound.closestIntersection(ray);
...
template <size_t N> BoundingBox<double, N> Box<N>::boundingBoxLocal() const { return bound; }

template <size_t N> typename Box<N>::Builder Box<N>::builder() { return Builder(); }

template <size_t N> typename Box<N>::Builder &Box<N>::Builder::withLowerCorner(const Vector<double, N> &pt) {
  _lowerCorner = pt;
  return *this;
}

template <size_t N> typename Box<N>::Builder &Box<N>::Builder::withUpperCorner(const Vector<double, N> &pt) {
  _upperCorner = pt;
  return *this;
}

template <size_t N> typename Box<N>::Builder &Box<N>::Builder::withBoundingBox(const BoundingBox<double, N> &bbox) {
  _lowerCorner = bbox.lowerCorner;
  _upperCorner = bbox.upperCorner;
  return *this;
}

template <size_t N> Box<N> Box<N>::Builder::build() const {
  return Box<N>(_lowerCorner, _upperCorner, _transform, _isNormalFlipped);
}

template <size_t N> std::shared_ptr<Box<N>> Box<N>::Builder::makeShared() const {
  return std::shared_ptr<Box<N>>(new Box<N>(_lowerCorner, _upperCorner, _transform, _isNormalFlipped),
