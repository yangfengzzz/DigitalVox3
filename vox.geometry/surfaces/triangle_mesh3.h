...
public:
  ...
  //! Default constructor.
  explicit TriangleMesh3(const Transform3 &transform = Transform3(), bool isNormalFlipped = false);

  //! Constructs mesh with points, normals, uvs, and their indices.
  TriangleMesh3(PointArray points, NormalArray normals, UvArray uvs, IndexArray pointIndices, IndexArray normalIndices,
                IndexArray uvIndices, const Transform3 &transform_ = Transform3(), bool isNormalFlipped = false);
  ...
  void addPointNormalTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newNormalIndices);

  //! Adds a triangle with point, normal, and UV.
  void addPointUvNormalTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newUvIndices,
                                const Vector3UZ &newNormalIndices);

  //! Adds a triangle with point and UV.
  void addPointUvTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newUvIndices);
...
private:
  ...
  [[nodiscard]] double windingNumber(const Vector3D &queryPoint, size_t triIndex) const;

  [[nodiscard]] double fastWindingNumber(const Vector3D &queryPoint, double accuracy) const;

  [[nodiscard]] double fastWindingNumber(const Vector3D &queryPoint, size_t rootNodeIndex, double accuracy) const;
};
