...
WindingNumberGatherData PostOrderTraversal(const Bvh3<size_t> &bvh, size_t nodeIndex, const GatherFunc &visitorFunc,
...
TriangleMesh3::TriangleMesh3(PointArray points, NormalArray normals, UvArray uvs, IndexArray pointIndices,
                             IndexArray normalIndices, IndexArray uvIndices, const Transform3 &transform_,
                             bool isNormalFlipped_)
    : Surface3(transform_, isNormalFlipped_), _points(std::move(points)), _normals(std::move(normals)),
      _uvs(std::move(uvs)), _pointIndices(std::move(pointIndices)), _normalIndices(std::move(normalIndices)),
...
const Vector3UZ &TriangleMesh3::pointIndex(size_t i) const { return _pointIndices[i]; }

Vector3UZ &TriangleMesh3::pointIndex(size_t i) { return _pointIndices[i]; }

const Vector3UZ &TriangleMesh3::normalIndex(size_t i) const { return _normalIndices[i]; }
...
void TriangleMesh3::addPointNormalTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newNormalIndices) {
  _pointIndices.append(newPointIndices);
  _normalIndices.append(newNormalIndices);
...
void TriangleMesh3::addPointUvNormalTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newUvIndices,
...
void TriangleMesh3::addPointUvTriangle(const Vector3UZ &newPointIndices, const Vector3UZ &newUvIndices) {
  _pointIndices.append(newPointIndices);
  _uvIndices.append(newUvIndices);
...
bool TriangleMesh3::isInsideLocal(const Vector3D &otherPoint) const {
  return fastWindingNumber(otherPoint, DEFAULT_FAST_WINDING_NUMBER_ACCURACY) > 0.5;
}
