...
//!
template <size_t N> class Box final : public Surface<N> {
public:
  class Builder;
  ...
  BoundingBox<double, N> bound = (BoundingBox<double, N>(Vector<double, N>(), Vector<double, N>::makeConstant(1.0)));

  //! Constructs (0, 0, ...) x (1, 1, ...) box.
  explicit Box(const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);

  //! Constructs a box with given \p lowerCorner and \p upperCorner.
  Box(const Vector<double, N> &lowerCorner, const Vector<double, N> &upperCorner,
      const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);

  //! Constructs a box with BoundingBox instance.
  explicit Box(const BoundingBox<double, N> &boundingBox, const Transform<N> &transform = Transform<N>(),
...
//!
template <size_t N> class Box<N>::Builder final : public SurfaceBuilderBase<N, typename Box<N>::Builder> {
public:
