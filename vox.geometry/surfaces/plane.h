...
//!
template <size_t N> class Plane final : public Surface<N> {
public:
  class Builder;
  ...
  //! (y-axis).
  explicit Plane(const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);

  //! Constructs a plane that cross \p point with surface normal \p normal.
  Plane(const Vector<double, N> &normal, const Vector<double, N> &point, const Transform<N> &transform = Transform<N>(),
...
//!
template <size_t N> class Plane<N>::Builder final : public SurfaceBuilderBase<N, typename Plane<N>::Builder> {
public:
