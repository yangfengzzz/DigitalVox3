...
//!
template <size_t N> class Sphere final : public Surface<N> {
public:
  class Builder;
  ...
  explicit Sphere(const Transform<N> &transform = Transform<N>(), bool isNormalFlipped = false);

  //! Constructs a sphere with \p center and \p radius.
  Sphere(const Vector<double, N> &center, double radius, const Transform<N> &transform = Transform<N>(),
...
//!
template <size_t N> class Sphere<N>::Builder final : public SurfaceBuilderBase<N, typename Sphere<N>::Builder> {
public:
