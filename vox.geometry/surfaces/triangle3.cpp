...
inline Vector3D closestNormalOnLine(const Vector3D &v0, const Vector3D &v1, const Vector3D &n0, const Vector3D &n1,
                                    const Vector3D &pt) {
  const double lenSquared = (v1 - v0).lengthSquared();
  if (lenSquared < std::numeric_limits<double>::epsilon()) {
    return n0;
...
Triangle3::Triangle3(std::array<Vector3D, 3> newPoints, std::array<Vector3D, 3> newNormals,
                     std::array<Vector2D, 3> newUvs, const Transform3 &transform_, bool isNormalFlipped_)
    : Surface3(transform_, isNormalFlipped_), points(std::move(newPoints)), normals(std::move(newNormals)),
      uvs(std::move(newUvs)) {}
