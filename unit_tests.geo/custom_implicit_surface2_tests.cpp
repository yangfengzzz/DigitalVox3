...
#include "../vox.geometry/implicit_surfaces/custom_implicit_surface.h"
...
TEST(CustomImplicitSurface2, SignedDistance) {
  CustomImplicitSurface2 cis([](const Vector2D &pt) { return (pt - Vector2D(0.5, 0.5)).length() - 0.25; },
...
TEST(CustomImplicitSurface2, CloseestPoint) {
  ...
  CustomImplicitSurface2 cis1([&](const Vector2D &pt) { return refSurf.signedDistance(pt); },
                              BoundingBox2D({0, 0}, {1, 1}), 1e-3);
