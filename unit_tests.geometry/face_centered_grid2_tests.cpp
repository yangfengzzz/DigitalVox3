...
TEST(FaceCenteredGrid2, Constructors) {
  ...
  grid2.forEachUIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(5.0, grid2.u(idx)); });
  grid2.forEachVIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(6.0, grid2.v(idx)); });
  ...
  grid3.forEachUIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(5.0, grid3.u(idx)); });
  grid3.forEachVIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(6.0, grid3.v(idx)); });
}
...
TEST(FaceCenteredGrid2, Builder) {
  {
    ...
    auto grid = builder.build(Vector2UZ(5, 2), Vector2D(2.0, 4.0), Vector2D(-1.0, 2.0), Vector2D(3.0, 5.0));
    ...
    faceCenteredGrid->forEachUIndex(
        [&faceCenteredGrid](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(3.0, faceCenteredGrid->u(idx)); });
    faceCenteredGrid->forEachVIndex(
        [&faceCenteredGrid](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(5.0, faceCenteredGrid->v(idx)); });
  }

  {
    ...
    grid.forEachUIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(3.0, grid.u(idx)); });
    grid.forEachVIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(5.0, grid.v(idx)); });
  }
}
...
TEST(FaceCenteredGrid2, Serialization) {
  ...
  grid1.forEachUIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(grid1.u(idx), grid2.u(idx)); });

  grid1.forEachVIndex([&](const Vector2UZ &idx) { EXPECT_DOUBLE_EQ(grid1.v(idx), grid2.v(idx)); });
