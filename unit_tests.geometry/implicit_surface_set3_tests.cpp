...
TEST(ImplicitSurfaceSet3, ClosestIntersection) {
  ...
  SurfaceRayIntersection3 result0 =
      sset.closestIntersection(Ray3D(Vector3D(1, 4, 5), Vector3D(-1, -1, -1).normalized()));
  ...
  SurfaceRayIntersection3 result1 =
      sset.closestIntersection(Ray3D(Vector3D(1, 2.5, 6), Vector3D(-1, -1, 1).normalized()));
  ...
  SurfaceRayIntersection3 result2 =
      sset.closestIntersection(Ray3D(Vector3D(1, 1, 2), Vector3D(-1, -1, -1).normalized()));
...
TEST(ImplicitSurfaceSet3, MixedBoundTypes) {
  ...
  const auto plane = Plane3::Builder{}.withNormal({0, 1, 0}).withPoint({0, 0.25 * domain.height(), 0}).makeShared();

  const auto sphere = Sphere3::Builder{}.withCenter(domain.midPoint()).withRadius(0.15 * domain.width()).makeShared();

  const auto surfaceSet =
      ImplicitSurfaceSet3::Builder{}.withExplicitSurfaces(Array1<Surface3Ptr>{plane, sphere}).makeShared();
