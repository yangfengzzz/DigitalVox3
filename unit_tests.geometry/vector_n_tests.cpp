...
TEST(VectorN, Constructors) {
  VectorND empty;
  EXPECT_EQ(0u, empty.rows());

  VectorND vec({1.0, 4.0, 3.0, -5.0, 2.0});

  EXPECT_EQ(5u, vec.rows());
  EXPECT_EQ(1.0, vec[0]);
  EXPECT_EQ(4.0, vec[1]);
  EXPECT_EQ(3.0, vec[2]);
  EXPECT_EQ(-5.0, vec[3]);
