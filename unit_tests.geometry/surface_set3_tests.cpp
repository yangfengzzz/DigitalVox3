...
TEST(SurfaceSet3, Constructors) {
  ...
  SurfaceSet3 sset2(Array1<Surface3Ptr>{sph1, sph2, sph3}, Transform3(), false);
  ...
  SurfaceSet3 sset3(Array1<Surface3Ptr>{sph1, sph2, sph3},
                    Transform3(Vector3D(1, 2, 3), Orientation<3>{QuaternionD({1, 0, 0}, 0.5)}), false);
  EXPECT_EQ(Vector3D(1, 2, 3), sset3.transform.translation());
  EXPECT_EQ(QuaternionD({1, 0, 0}, 0.5), sset3.transform.orientation().rotation());
}
...
TEST(SurfaceSet3, ClosestPoint) {
  ...
  for (size_t i = 0; i < numSamples / 2; ++i) {
    auto sph =
        Sphere3::builder().withRadius(0.01).withCenter({0, 0, 0}).withTranslation(getSamplePoints3()[i]).makeShared();
...
TEST(SurfaceSet3, MixedBoundTypes) {
  ...
  auto surfaceSet = SurfaceSet3::builder().withSurfaces(Array1<Surface3Ptr>{plane, sphere}).makeShared();
...
TEST(SurfaceSet3, IsValidGeometry) {
  ...
  auto surfaceSet2 = SurfaceSet3::builder().withSurfaces(Array1<Surface3Ptr>{plane, sphere}).makeShared();
...
TEST(SurfaceSet3, IsInside) {
  ...
  auto surfaceSet = SurfaceSet3::builder()
                        ...
                        .withTransform(Transform3(offset, Orientation<3>{QuaternionD()}))
                        .makeShared();
...
TEST(SurfaceSet3, UpdateQueryEngine) {
  ...
  auto surfaceSet = SurfaceSet3::Builder{}
                        ...
                        .withTransform(Transform3{{1.0, 2.0, -1.0}, Orientation<3>{QuaternionD{}}})
                        .makeShared();

  const auto bbox1 = surfaceSet->boundingBox();
  EXPECT_BOUNDING_BOX2_EQ(BoundingBox3D({-0.5, 2.5, 0.5}, {0.5, 3.5, 1.5}), bbox1);

  surfaceSet->transform = Transform3{{3.0, -4.0, 7.0}, Orientation<3>{QuaternionD{}}};
  ...
  sphere->transform = Transform3{{-6.0, 9.0, 2.0}, Orientation<3>{QuaternionD{}}};
  surfaceSet->updateQueryEngine();
