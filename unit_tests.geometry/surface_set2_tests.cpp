...
TEST(SurfaceSet2, Constructors) {
  ...
  SurfaceSet2 sset2(Array1<Surface2Ptr>{sph1, sph2, sph3}, Transform2(), false);
  ...
  SurfaceSet2 sset3(Array1<Surface2Ptr>{sph1, sph2, sph3}, Transform2(Vector2D(1, 2), Orientation<2>{0.5}), false);
...
TEST(SurfaceSet2, ClosestPoint) {
  ...
  for (size_t i = 0; i < numSamples / 2; ++i) {
    auto sph =
        Sphere2::builder().withRadius(0.01).withCenter({0, 0, 0}).withTranslation(getSamplePoints2()[i]).makeShared();
...
TEST(SurfaceSet2, MixedBoundTypes) {
  ...
  auto surfaceSet = SurfaceSet2::builder().withSurfaces(Array1<Surface2Ptr>{plane, sphere}).makeShared();
...
TEST(SurfaceSet2, IsValidGeometry) {
  ...
  auto surfaceSet2 = SurfaceSet2::builder().withSurfaces(Array1<Surface2Ptr>{plane, sphere}).makeShared();
...
TEST(SurfaceSet2, IsInside) {
  ...
  auto surfaceSet = SurfaceSet2::builder()
                        ...
                        .withTransform(Transform2(offset, Orientation<2>{0.0}))
                        .makeShared();
...
TEST(SurfaceSet2, UpdateQueryEngine) {
  ...
  auto surfaceSet = SurfaceSet2::Builder{}
                        ...
                        .withTransform(Transform2{{1.0, 2.0}, Orientation<2>{0.0}})
                        .makeShared();

  const auto bbox1 = surfaceSet->boundingBox();
  EXPECT_BOUNDING_BOX2_EQ(BoundingBox2D({-0.5, 2.5}, {0.5, 3.5}), bbox1);

  surfaceSet->transform = Transform2{{3.0, -4.0}, Orientation<2>{0.0}};
  ...
  sphere->transform = Transform2{{-6.0, 9.0}, Orientation<2>{0.0}};
  surfaceSet->updateQueryEngine();
