...
TEST(ImplicitSurfaceSet2, MixedBoundTypes) {
  ...
  const auto plane = Plane2::Builder{}.withNormal({0, 1}).withPoint({0.0, 0.25 * domain.height()}).makeShared();

  const auto sphere = Sphere2::Builder{}.withCenter(domain.midPoint()).withRadius(0.15 * domain.width()).makeShared();

  const auto surfaceSet =
      ImplicitSurfaceSet2::Builder{}.withExplicitSurfaces(Array1<Surface2Ptr>{plane, sphere}).makeShared();
