...
TEST(NearestArraySampler1, Sample) {
  {
    ...
    NearestArraySampler1<double> sampler(grid.view(), {gridSpacing}, {gridOrigin});
...
TEST(LinearArraySampler1, Sample) {
  {
    Array1<double> grid({1.0, 2.0, 3.0, 4.0});
    double gridSpacing = 1.0, gridOrigin = 0.0;
    LinearArraySampler1<double> sampler(grid.view(), {gridSpacing}, {gridOrigin});
...
TEST(CubicArraySampler1, Sample) {
  Array1<double> grid({1.0, 2.0, 3.0, 4.0});
  double gridSpacing = 1.0, gridOrigin = 0.0;
  MonotonicCatmullRomArraySampler1<double> sampler(grid.view(), {gridSpacing}, {gridOrigin});
...
TEST(NearestArraySampler2, Sample) {
  {
    Array2<double> grid(
        {{1.0, 2.0, 3.0, 4.0}, {2.0, 3.0, 4.0, 5.0}, {3.0, 4.0, 5.0, 6.0}, {4.0, 5.0, 6.0, 7.0}, {5.0, 6.0, 7.0, 8.0}});
    Vector2D gridSpacing(1.0, 1.0), gridOrigin;
    NearestArraySampler2<double> sampler(grid.view(), gridSpacing, gridOrigin);
...
TEST(LinearArraySampler2, Sample) {
  // From simple grid
  {
    Array2<double> grid(
        {{1.0, 2.0, 3.0, 4.0}, {2.0, 3.0, 4.0, 5.0}, {3.0, 4.0, 5.0, 6.0}, {4.0, 5.0, 6.0, 7.0}, {5.0, 6.0, 7.0, 8.0}});
    Vector2D gridSpacing(1.0, 1.0), gridOrigin;
    LinearArraySampler2<double> sampler(grid.view(), gridSpacing, gridOrigin);
...
TEST(LinearArraySampler2, GetCoordinatesAndWeights) {
  ...
  // From more complex grid
  {
    ...
    sampler.getCoordinatesAndWeights(Vector2D(-0.5, 0.375), indices, weights);
    EXPECT_EQ(indices[0], Vector2UZ(1, 3));
    EXPECT_EQ(indices[1], Vector2UZ(2, 3));
    EXPECT_EQ(indices[2], Vector2UZ(1, 4));
    EXPECT_EQ(indices[3], Vector2UZ(2, 4));
...
TEST(LinearArraySampler2, GetCoordinatesAndGradientWeights) {
  // From simple grid
  {
    ...
    sampler.getCoordinatesAndGradientWeights(Vector2D(0.5, 0.5), indices, weights);
    EXPECT_EQ(indices[0], Vector2UZ(0, 0));
    EXPECT_EQ(indices[1], Vector2UZ(1, 0));
    EXPECT_EQ(indices[2], Vector2UZ(0, 1));
    EXPECT_EQ(indices[3], Vector2UZ(1, 1));
    ...
    sampler.getCoordinatesAndGradientWeights(Vector2D(1.3, 4.0), indices, weights);
    EXPECT_EQ(indices[0], Vector2UZ(1, 3));
    EXPECT_EQ(indices[1], Vector2UZ(2, 3));
    EXPECT_EQ(indices[2], Vector2UZ(1, 4));
    EXPECT_EQ(indices[3], Vector2UZ(2, 4));
  ...
  // From more complex grid
  {
    ...
    sampler.getCoordinatesAndGradientWeights(Vector2D(-0.5, 0.375), indices, weights);
    EXPECT_EQ(indices[0], Vector2UZ(1, 3));
    EXPECT_EQ(indices[1], Vector2UZ(2, 3));
    EXPECT_EQ(indices[2], Vector2UZ(1, 4));
...
TEST(CubicArraySampler2, Sample) {
  Array2<double> grid(
      {{1.0, 2.0, 3.0, 4.0}, {2.0, 3.0, 4.0, 5.0}, {3.0, 4.0, 5.0, 6.0}, {4.0, 5.0, 6.0, 7.0}, {5.0, 6.0, 7.0, 8.0}});
  Vector2D gridSpacing(1.0, 1.0), gridOrigin;
  MonotonicCatmullRomArraySampler2<double> sampler(grid.view(), gridSpacing, gridOrigin);
...
TEST(CubicArraySampler3, Sample) {
  ...
  MonotonicCatmullRomArraySampler3<double> sampler(grid.view(), gridSpacing, gridOrigin);
