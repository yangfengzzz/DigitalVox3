...
bool BuildVertices(FbxMesh* _fbx_mesh,
                   vox::offline::loader::FbxSystemConverter* _converter,
                   ControlPointsRemap* _remap,
                   vox::offline::loader::Mesh* _output_mesh) {
    ...
    // rejected.
    vox::offline::loader::Mesh::Part& part = _output_mesh->parts[0];
    part.positions.reserve(vertex_count *
                           vox::offline::loader::Mesh::Part::kPositionsCpnts);
    part.normals.reserve(vertex_count * vox::offline::loader::Mesh::Part::kNormalsCpnts);
    if (element_tangents) {
        part.tangents.reserve(vertex_count *
                              vox::offline::loader::Mesh::Part::kTangentsCpnts);
    }
    if (element_uvs) {
        part.uvs.reserve(vertex_count * vox::offline::loader::Mesh::Part::kUVsCpnts);
    }
    if (element_colors) {
        part.colors.reserve(vertex_count * vox::offline::loader::Mesh::Part::kColorsCpnts);
    }
    ...
    for (int p = 0; p < polygon_count; ++p) {
        ...
        for (int v = 0; v < 3; ++v, ++vertex_id) {
            ...
            // Get vertex position.
            const vox::math::Float3 position =
            ...
            const vox::math::Float3 normal = NormalizeSafe(
                                                           _converter->ConvertVector(src_normal), vox::math::Float3::y_axis());
            ...
            const vox::math::Float3 tangent3 = NormalizeSafe(
                                                             _converter->ConvertVector(src_tangent), vox::math::Float3::x_axis());
            const vox::math::Float4 tangent(tangent3,
            ...
            const vox::math::Float2 uv(static_cast<float>(src_uv[0]),
            ...
            const uint8_t color[4] = {
                static_cast<uint8_t>(
                                     vox::math::Clamp(0., src_color.mRed * 255., 255.)),
                static_cast<uint8_t>(
                                     vox::math::Clamp(0., src_color.mGreen * 255., 255.)),
                static_cast<uint8_t>(
                                     vox::math::Clamp(0., src_color.mBlue * 255., 255.)),
                static_cast<uint8_t>(
                                     vox::math::Clamp(0., src_color.mAlpha * 255., 255.)),
            };
            ...
            for (size_t r = 0; r < remap.size(); ++r) {
                ...
                if (!Compare(
                             &normal.x,
                             &part.normals[to_test * vox::offline::loader::Mesh::Part::kNormalsCpnts],
                             vox::offline::loader::Mesh::Part::kNormalsCpnts)) {
                ...
                if (element_uvs) {
                    if (!Compare(&uv.x,
                                 &part.uvs[to_test * vox::offline::loader::Mesh::Part::kUVsCpnts],
                                 vox::offline::loader::Mesh::Part::kUVsCpnts)) {
                        continue;  // Next vertex.
                    }
                }
                ...
                if (element_colors) {
                    if (!Compare(
                                 color,
                                 &part.colors[to_test * vox::offline::loader::Mesh::Part::kColorsCpnts],
                                 vox::offline::loader::Mesh::Part::kColorsCpnts)) {
                                     continue;  // Next vertex.
                                 }
                }
            ...
            if (redundant_with >= 0) {
                ...
                if ((part.positions.size() / 3) >
                    ...
                    vox::log::Err() << "Mesh uses too many vertices (> "
                    << std::numeric_limits<uint16_t>::max()
                    << ") to fit in the index "
                    "buffer."
                    << std::endl;
                ...
                uint16_t vertex_index = static_cast<uint16_t>(
                                                              part.positions.size() / vox::offline::loader::Mesh::Part::kPositionsCpnts);
...
bool BuildSkin(FbxMesh* _fbx_mesh,
    ...
    if (skin_count == 0) {
        vox::log::Err() << "No skin found." << std::endl;
        return false;
    }
    if (skin_count > 1) {
        vox::log::Log()
    ...
    if (skinning_type != FbxSkin::eRigid && skinning_type != FbxSkin::eLinear) {
        vox::log::Err() << "Unsupported skinning type" << std::endl;
        return false;
    }
    ...
    for (int i = 0; i < _skeleton.num_joints(); ++i) {
        _output_mesh->inverse_bind_poses[i] = vox::math::Float4x4::identity();
    }
    ...
    for (int cl = 0; cl < cluster_count; ++cl) {
        ...
        if (!node) {
            vox::log::Log() << "No node linked to cluster " << cluster->GetName()
            << "." << std::endl;
            continue;
        }
        ...
        if (mode != FbxCluster::eNormalize) {
            vox::log::Err() << "Unsupported link mode for joint " << node->GetName()
            << "." << std::endl;
            return false;
        }
        ...
        if (it == joints_map.end()) {
            vox::log::Err() << "Required joint " << node->GetName()
    ...
    for (size_t i = 0; i < vertex_count; ++i) {
        ...
        max_influences = vox::math::Max(max_influences, inv.size());
    ...
    if (vertex_isnt_influenced) {
        vox::log::Err() << "At least one vertex isn't influenced by any joints."
        << std::endl;
    }
...
bool LimitInfluences(vox::offline::loader::Mesh& _skinned_mesh, int _limit) {
    assert(_skinned_mesh.parts.size() == 1);
    
    vox::offline::loader::Mesh::Part& in_part = _skinned_mesh.parts.front();
...
// matrices.
bool RemapIndices(vox::offline::loader::Mesh* _skinned_mesh) {
    assert(_skinned_mesh->parts.size() == 1);
    ...
    vox::offline::loader::Mesh::Part::JointIndices local_indices = in_part.joint_indices;
    ...
    // are set to 0.
    vox::offline::loader::Mesh::Part::JointIndices original_remap(
                                                                  _skinned_mesh->num_joints(), 0);
    for (size_t i = 0; i < local_indices.size(); ++i) {
        original_remap[local_indices[i]] =
        static_cast<vox::offline::loader::Mesh::Part::JointIndices::value_type>(i);
    }
...
bool SplitParts(const vox::offline::loader::Mesh& _skinned_mesh,
                vox::offline::loader::Mesh* _partitionned_mesh) {
    assert(_skinned_mesh.parts.size() == 1);
    assert(_partitionned_mesh->parts.size() == 0);
    
    const vox::offline::loader::Mesh::Part& in_part = _skinned_mesh.parts.front();
    const size_t vertex_count = in_part.vertex_count();
    ...
    for (int i = 0; i < max_influences; ++i) {
        ...
        vox::offline::loader::Mesh::Part& out_part = _partitionned_mesh->parts.back();
        ...
        for (size_t j = 0; j < bucket_vertex_count; ++j) {
            ...
            // Fills positions.
            float* out_pos =
            &out_part.positions[j * vox::offline::loader::Mesh::Part::kPositionsCpnts];
            const float* in_pos =
            ...
            // Fills normals.
            float* out_normal =
            &out_part.normals[j * vox::offline::loader::Mesh::Part::kNormalsCpnts];
            ...
            // Fills uvs.
            if (in_part.uvs.size()) {
                float* out_uv = &out_part.uvs[j * vox::offline::loader::Mesh::Part::kUVsCpnts];
                const float* in_uv =
                &in_part
                    .uvs[bucket_vertex_index * vox::offline::loader::Mesh::Part::kUVsCpnts];
                out_uv[0] = in_uv[0];
                out_uv[1] = in_uv[1];
            }
            // Fills colors.
            if (in_part.colors.size()) {
                uint8_t* out_color =
                &out_part.colors[j * vox::offline::loader::Mesh::Part::kColorsCpnts];
            ...
            if (in_part.tangents.size()) {
                float* out_tangent =
                &out_part.tangents[j * vox::offline::loader::Mesh::Part::kTangentsCpnts];
...
// weights equals 1).
bool StripWeights(vox::offline::loader::Mesh* _mesh) {
    for (size_t i = 0; i < _mesh->parts.size(); ++i) {
        vox::offline::loader::Mesh::Part& part = _mesh->parts[i];
